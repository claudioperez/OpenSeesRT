...
proc BeamWithSteel01HingesWSection2d {eleTag iNode jNode sectType E Fy b transfTag args} {
  ...
  set crd11 [nodeCoord $iNode 1]; set crd12 [nodeCoord $iNode 2];
  set crd21 [nodeCoord $jNode 1]; set crd22 [nodeCoord $jNode 2];

  set diffX [expr ($crd21-$crd11)/3.];
  set diffY [expr ($crd22-$crd12)/3.];

  set hingeEnd1 1
  node $eleTag$hingeEnd1 $crd11 $crd12 -dispLoc [expr $crd11+$diffX] [expr $crd12+$diffY]
  equalDOF $iNode $eleTag$hingeEnd1 1 2

  set hingeEnd2 2
  node $eleTag$hingeEnd2 $crd21 $crd22 -dispLoc [expr $crd21-$diffX] [expr $crd22-$diffY]
  equalDOF $jNode $eleTag$hingeEnd2 1 2
...
#    puts stderr "YY orientation not handled - uses XX!"
  ...
  set Com_Type [lindex $args [expr $loc+1]]
  }
  ...
  set H $L
  if {[lsearch $args "-H"] != -1} {
  set loc [lsearch $args "-H"]
  set H [lindex $args [expr $loc+1]]
  }
...
#    puts stderr "YY orientation not handled - uses XX!"
  ...
  if {[lsearch $args "-metric"] != -1} {
  SteelSquareHSSMR02 $eleTag $E $Fy $H $N $sectType -metric -nFactor $nFactor -matType $matType
  } else {
...
proc ElasticHSSection2d {sectTag sectType E args} {
  ...
  if {$Orient == "YY" } {
    section Elastic $sectTag $E $A $Iy
  } else {
    section Elastic $sectTag $E $A $Ix
  }
  set found 1
  }
...
proc SteelSquareHSSMR02 {matTag E Fy H N sectType args} {
 ...
 ###############################################################################################
  ...
  set  t [expr [lindex $propList 4]*$in]
  set Ix [expr [lindex $propList 5]*$in*$in*$in*$in]
  set Zx [expr [lindex $propList 6]*$in*$in*$in]
  set Sx [expr [lindex $propList 7]*$in*$in*$in]
  set rx [expr [lindex $propList 8]*$in]
  set Iy [expr [lindex $propList 9]*$in*$in*$in*$in]
...
#  puts stderr "ORIGINAL:        uniaxialMaterial MultiLinear $matTag $e1p $s1p $e2p $s2p $e3p $s3p [expr $e3p+10.] 0.        "
  ...
  set capSlopeMember [expr -($s1p + $ekhardPos * $theta_p_P)/($theta_pc_P*$K)]
...
#        uniaxialMaterial Steel01 $matTag $Ke $s1p $alphaPos
...
proc SteelWSectionATC41 {matTag E Fy L sectType args} {
 ...
 ###############################################################################################
  ...
  if {([expr $bf/(2.*$tf)] < [expr 52.0/sqrt($Fy)]) && ([expr $h/$tw]< [expr 418/sqrt($Fy)])} {
  set a [expr 9.0*$thetaY]
  set b [expr 11*$thetaY]
  set Mr [0.6*$My]
  } else {
  set a [expr 4.0*$thetaY]
...
# define panel zone nodes
  ...
  set node_xy6  [expr ($node_xy01-1) + 61];        # btm center of joint
  set node_xy7  [expr ($node_xy01-1) + 71];        # top center of joint
...
W14X108 "32.0 14.3 14.6 0.525 0.860 1240 447 192 173 6.22 92.7 61.2 3.73 7.12"
...
# AISC_Manual_Label "W A h b tdes Ix Zx Sx rx Iy Zy Sy ry J C
# Added built-up hollow square sections
# AISC Manual sections
array set HSSection {
HSS15X15X2                "378.5        104.0        15.0        15.0        2.0        2998.7        511.0        399.8        5.370        2998.7        511.0        399.8        5.370        6750.0        676.0"
HSS15X15X5/4        "250.2        68.8        15.0        15.0        1.250        2184.2        355.5        291.2        5.637        2184.2        355.5        291.2        5.637        4218.8        472.7"
HSS15X15X1                "203.8        56.0        15.0        15.0        1.0        1838.7        294.5        245.2        5.730        1838.7        294.5        245.2        5.730        3375.0        392.0"
HSS15X15X3/4        "155.6        42.8        15.0        15.0        0.750        1450.8        228.7        193.4        5.826        1450.8        228.7        193.4        5.826        2531.3        304.6"
HSS15X15X1/2        "105.5        29.0        15.0        15.0        0.5        1017.4        157.8        135.7        5.923        1017.4        157.8        135.7        5.923        1687.5        210.3"
xq1                        "126.4        34.7        16.0        16.0        0.563        1381.5        201.2        172.7        6.307        1381.5        201.2        172.7        6.307        2304.0        268.1"
...
proc write_displacements {file_name {name 1}} {
...
#
# Axial load ratio due to gravity load
#
set       121b 0.10423; #  Column at STORY-1 and AXIS-1 (Bottom)
set       121t 0.10423; #  Column at STORY-1 and AXIS-1 (Top)
set       122b 0.06560; #  Column at STORY-1 and AXIS-2 (Bottom)
set       122t 0.06560; #  Column at STORY-1 and AXIS-2 (Top)
set       123b 0.06560; #  Column at STORY-1 and AXIS-3 (Bottom)
set       123t 0.06560; #  Column at STORY-1 and AXIS-3 (Top)
set       124b 0.10423; #  Column at STORY-1 and AXIS-4 (Bottom)
set       124t 0.10423; #  Column at STORY-1 and AXIS-4 (Top)
set       231b 0.09892; #  Column at STORY-2 and AXIS-1 (Bottom)
set       231t 0.09892; #  Column at STORY-2 and AXIS-1 (Top)
set       232b 0.06227; #  Column at STORY-2 and AXIS-2 (Bottom)
set       232t 0.06227; #  Column at STORY-2 and AXIS-2 (Top)
set       233b 0.06227; #  Column at STORY-2 and AXIS-3 (Bottom)
set       233t 0.06227; #  Column at STORY-2 and AXIS-3 (Top)
set       234b 0.09892; #  Column at STORY-2 and AXIS-4 (Bottom)
set       234t 0.09892; #  Column at STORY-2 and AXIS-4 (Top)
set       341b 0.09367; #  Column at STORY-3 and AXIS-1 (Bottom)
set       341t 0.10304; #  Column at STORY-3 and AXIS-1 (Top)
set       342b 0.05896; #  Column at STORY-3 and AXIS-2 (Bottom)
set       342t 0.06398; #  Column at STORY-3 and AXIS-2 (Top)
set       343b 0.05896; #  Column at STORY-3 and AXIS-3 (Bottom)
set       343t 0.06398; #  Column at STORY-3 and AXIS-3 (Top)
set       344b 0.09367; #  Column at STORY-3 and AXIS-4 (Bottom)
set       344t 0.10304; #  Column at STORY-3 and AXIS-4 (Top)
set       451b 0.09726; #  Column at STORY-4 and AXIS-1 (Bottom)
set       451t 0.09726; #  Column at STORY-4 and AXIS-1 (Top)
set       452b 0.06039; #  Column at STORY-4 and AXIS-2 (Bottom)
set       452t 0.06039; #  Column at STORY-4 and AXIS-2 (Top)
set       453b 0.06039; #  Column at STORY-4 and AXIS-3 (Bottom)
set       453t 0.06039; #  Column at STORY-4 and AXIS-3 (Top)
set       454b 0.09726; #  Column at STORY-4 and AXIS-4 (Bottom)
set       454t 0.09726; #  Column at STORY-4 and AXIS-4 (Top)
set       561b 0.09148; #  Column at STORY-5 and AXIS-1 (Bottom)
set       561t 0.09148; #  Column at STORY-5 and AXIS-1 (Top)
set       562b 0.05680; #  Column at STORY-5 and AXIS-2 (Bottom)
set       562t 0.06265; #  Column at STORY-5 and AXIS-2 (Top)
set       563b 0.05680; #  Column at STORY-5 and AXIS-3 (Bottom)
set       563t 0.06265; #  Column at STORY-5 and AXIS-3 (Top)
set       564b 0.09148; #  Column at STORY-5 and AXIS-4 (Bottom)
set       564t 0.09148; #  Column at STORY-5 and AXIS-4 (Top)
set       671b 0.08570; #  Column at STORY-6 and AXIS-1 (Bottom)
set       671t 0.08570; #  Column at STORY-6 and AXIS-1 (Top)
set       672b 0.05870; #  Column at STORY-6 and AXIS-2 (Bottom)
set       672t 0.05870; #  Column at STORY-6 and AXIS-2 (Top)
set       673b 0.05870; #  Column at STORY-6 and AXIS-3 (Bottom)
set       673t 0.05870; #  Column at STORY-6 and AXIS-3 (Top)
set       674b 0.08570; #  Column at STORY-6 and AXIS-4 (Bottom)
set       674t 0.08570; #  Column at STORY-6 and AXIS-4 (Top)
set       781b 0.07992; #  Column at STORY-7 and AXIS-1 (Bottom)
set       781t 0.09787; #  Column at STORY-7 and AXIS-1 (Top)
set       782b 0.05474; #  Column at STORY-7 and AXIS-2 (Bottom)
set       782t 0.06122; #  Column at STORY-7 and AXIS-2 (Top)
set       783b 0.05474; #  Column at STORY-7 and AXIS-3 (Bottom)
set       783t 0.06122; #  Column at STORY-7 and AXIS-3 (Top)
set       784b 0.07992; #  Column at STORY-7 and AXIS-4 (Bottom)
set       784t 0.09787; #  Column at STORY-7 and AXIS-4 (Top)
set       891b 0.09079; #  Column at STORY-8 and AXIS-1 (Bottom)
set       891t 0.09079; #  Column at STORY-8 and AXIS-1 (Top)
set       892b 0.05679; #  Column at STORY-8 and AXIS-2 (Bottom)
set       892t 0.05679; #  Column at STORY-8 and AXIS-2 (Top)
set       893b 0.05679; #  Column at STORY-8 and AXIS-3 (Bottom)
set       893t 0.05679; #  Column at STORY-8 and AXIS-3 (Top)
set       894b 0.09079; #  Column at STORY-8 and AXIS-4 (Bottom)
set       894t 0.09079; #  Column at STORY-8 and AXIS-4 (Top)
set       9101b 0.08371; #  Column at STORY-9 and AXIS-1 (Bottom)
set       9101t 0.08371; #  Column at STORY-9 and AXIS-1 (Top)
set       9102b 0.05236; #  Column at STORY-9 and AXIS-2 (Bottom)
set       9102t 0.05743; #  Column at STORY-9 and AXIS-2 (Top)
set       9103b 0.05236; #  Column at STORY-9 and AXIS-3 (Bottom)
set       9103t 0.05743; #  Column at STORY-9 and AXIS-3 (Top)
set       9104b 0.08371; #  Column at STORY-9 and AXIS-4 (Bottom)
set       9104t 0.08371; #  Column at STORY-9 and AXIS-4 (Top)
set       10111b 0.07663; #  Column at STORY-10 and AXIS-1 (Bottom)
set       10111t 0.07663; #  Column at STORY-10 and AXIS-1 (Top)
set       10112b 0.05257; #  Column at STORY-10 and AXIS-2 (Bottom)
set       10112t 0.05257; #  Column at STORY-10 and AXIS-2 (Top)
set       10113b 0.05257; #  Column at STORY-10 and AXIS-3 (Bottom)
set       10113t 0.05257; #  Column at STORY-10 and AXIS-3 (Top)
set       10114b 0.07663; #  Column at STORY-10 and AXIS-4 (Bottom)
set       10114t 0.07663; #  Column at STORY-10 and AXIS-4 (Top)
set       11121b 0.06956; #  Column at STORY-11 and AXIS-1 (Bottom)
set       11121t 0.08310; #  Column at STORY-11 and AXIS-1 (Top)
set       11122b 0.04772; #  Column at STORY-11 and AXIS-2 (Bottom)
set       11122t 0.05219; #  Column at STORY-11 and AXIS-2 (Top)
set       11123b 0.04772; #  Column at STORY-11 and AXIS-3 (Bottom)
set       11123t 0.05219; #  Column at STORY-11 and AXIS-3 (Top)
set       11124b 0.06956; #  Column at STORY-11 and AXIS-4 (Bottom)
set       11124t 0.08310; #  Column at STORY-11 and AXIS-4 (Top)
set       12131b 0.07464; #  Column at STORY-12 and AXIS-1 (Bottom)
set       12131t 0.07464; #  Column at STORY-12 and AXIS-1 (Top)
set       12132b 0.04688; #  Column at STORY-12 and AXIS-2 (Bottom)
set       12132t 0.04688; #  Column at STORY-12 and AXIS-2 (Top)
set       12133b 0.04688; #  Column at STORY-12 and AXIS-3 (Bottom)
set       12133t 0.04688; #  Column at STORY-12 and AXIS-3 (Top)
set       12134b 0.07464; #  Column at STORY-12 and AXIS-4 (Bottom)
set       12134t 0.07464; #  Column at STORY-12 and AXIS-4 (Top)
set       13141b 0.06619; #  Column at STORY-13 and AXIS-1 (Bottom)
set       13141t 0.07260; #  Column at STORY-13 and AXIS-1 (Top)
set       13142b 0.04157; #  Column at STORY-13 and AXIS-2 (Bottom)
set       13142t 0.05220; #  Column at STORY-13 and AXIS-2 (Top)
set       13143b 0.04157; #  Column at STORY-13 and AXIS-3 (Bottom)
set       13143t 0.05220; #  Column at STORY-13 and AXIS-3 (Top)
set       13144b 0.06619; #  Column at STORY-13 and AXIS-4 (Bottom)
set       13144t 0.07260; #  Column at STORY-13 and AXIS-4 (Top)
set       14151b 0.06332; #  Column at STORY-14 and AXIS-1 (Bottom)
set       14151t 0.06332; #  Column at STORY-14 and AXIS-1 (Top)
set       14152b 0.04553; #  Column at STORY-14 and AXIS-2 (Bottom)
set       14152t 0.04553; #  Column at STORY-14 and AXIS-2 (Top)
set       14153b 0.04553; #  Column at STORY-14 and AXIS-3 (Bottom)
set       14153t 0.04553; #  Column at STORY-14 and AXIS-3 (Top)
set       14154b 0.06332; #  Column at STORY-14 and AXIS-4 (Bottom)
set       14154t 0.06332; #  Column at STORY-14 and AXIS-4 (Top)
set       15161b 0.05404; #  Column at STORY-15 and AXIS-1 (Bottom)
set       15161t 0.06567; #  Column at STORY-15 and AXIS-1 (Top)
set       15162b 0.03886; #  Column at STORY-15 and AXIS-2 (Bottom)
set       15162t 0.04406; #  Column at STORY-15 and AXIS-2 (Top)
set       15163b 0.03886; #  Column at STORY-15 and AXIS-3 (Bottom)
set       15163t 0.04406; #  Column at STORY-15 and AXIS-3 (Top)
set       15164b 0.05404; #  Column at STORY-15 and AXIS-4 (Bottom)
set       15164t 0.06567; #  Column at STORY-15 and AXIS-4 (Top)
set       16171b 0.05439; #  Column at STORY-16 and AXIS-1 (Bottom)
set       16171t 0.05439; #  Column at STORY-16 and AXIS-1 (Top)
set       16172b 0.03649; #  Column at STORY-16 and AXIS-2 (Bottom)
set       16172t 0.03649; #  Column at STORY-16 and AXIS-2 (Top)
set       16173b 0.03649; #  Column at STORY-16 and AXIS-3 (Bottom)
set       16173t 0.03649; #  Column at STORY-16 and AXIS-3 (Top)
set       16174b 0.05439; #  Column at STORY-16 and AXIS-4 (Bottom)
set       16174t 0.05439; #  Column at STORY-16 and AXIS-4 (Top)
set       17181b 0.04312; #  Column at STORY-17 and AXIS-1 (Bottom)
set       17181t 0.05200; #  Column at STORY-17 and AXIS-1 (Top)
set       17182b 0.02893; #  Column at STORY-17 and AXIS-2 (Bottom)
set       17182t 0.03445; #  Column at STORY-17 and AXIS-2 (Top)
set       17183b 0.02893; #  Column at STORY-17 and AXIS-3 (Bottom)
set       17183t 0.03445; #  Column at STORY-17 and AXIS-3 (Top)
set       17184b 0.04312; #  Column at STORY-17 and AXIS-4 (Bottom)
set       17184t 0.05200; #  Column at STORY-17 and AXIS-4 (Top)
set       18191b 0.03841; #  Column at STORY-18 and AXIS-1 (Bottom)
set       18191t 0.03841; #  Column at STORY-18 and AXIS-1 (Top)
set       18192b 0.02545; #  Column at STORY-18 and AXIS-2 (Bottom)
set       18192t 0.02545; #  Column at STORY-18 and AXIS-2 (Top)
set       18193b 0.02545; #  Column at STORY-18 and AXIS-3 (Bottom)
set       18193t 0.02545; #  Column at STORY-18 and AXIS-3 (Top)
set       18194b 0.03841; #  Column at STORY-18 and AXIS-4 (Bottom)
set       18194t 0.03841; #  Column at STORY-18 and AXIS-4 (Top)
set       19201b 0.02481; #  Column at STORY-19 and AXIS-1 (Bottom)
set       19201t 0.03632; #  Column at STORY-19 and AXIS-1 (Top)
set       19202b 0.01644; #  Column at STORY-19 and AXIS-2 (Bottom)
set       19202t 0.02486; #  Column at STORY-19 and AXIS-2 (Top)
set       19203b 0.01644; #  Column at STORY-19 and AXIS-3 (Bottom)
set       19203t 0.02486; #  Column at STORY-19 and AXIS-3 (Top)
set       19204b 0.02481; #  Column at STORY-19 and AXIS-4 (Bottom)
set       19204t 0.03632; #  Column at STORY-19 and AXIS-4 (Top)
set       20211b 0.01642; #  Column at STORY-20 and AXIS-1 (Bottom)
set       20211t 0.01642; #  Column at STORY-20 and AXIS-1 (Top)
set       20212b 0.01124; #  Column at STORY-20 and AXIS-2 (Bottom)
set       20212t 0.01124; #  Column at STORY-20 and AXIS-2 (Top)
set       20213b 0.01124; #  Column at STORY-20 and AXIS-3 (Bottom)
set       20213t 0.01124; #  Column at STORY-20 and AXIS-3 (Top)
set       20214b 0.01642; #  Column at STORY-20 and AXIS-4 (Bottom)
