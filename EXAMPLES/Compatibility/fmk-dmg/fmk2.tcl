...
set Nfactor    {$scaleFactor 0.0788}; # 0.0788=386.4/980.6*0.2
set motionName {CanogaNS LA3536PPhor};
set motionNameLIN {CanogaNS01};
set motionID 1

foreach scaleFactor {1.0e-4} {
  foreach damp {RI M} {
    if {[expr $counter % $np] == $pid} {
	    ...
	    set nfactor [lindex $Nfactor [expr $motionID -1]];
#     puts "nfactor = $nfactor"
	    set motion [lindex $motionName [expr $motionID -1] ];
      set LINfactor 1.0;
#     puts $motion
...
#     foreach colType {ComponentElement}  beamType {ComponentElement} ;
	    foreach colType {Force}  beamType {Force} {		
        set totalMass 0
...
#         }
        ...
        constraints Plain;	
        numberer RCM;		
        system   BandSPD; # Umfpack;	
        test     NormUnbalance 1.0e-6 10 $testPrint;
        ...
        constraints Plain
        numberer RCM
        system   BandSPD; # UmfPack; #UmfPack ProfileSPD;  # <----
...
#       puts " Period T3: [expr 2*$PI/sqrt([lindex $lambda 2])] sec [lindex $lambda 2]"
        ...
        if {$motion == "elCentro"} {
            ...
            timeSeries Path 10 -filePath $motion.txt -dt $dt -factor [expr ($g/9806.0*$nfactor)]
...
#       puts "TOTAL MASS: $totalMass"
        ...
        set recordAll "NO";  #<-----
        if {$recordAll == "YES"} {
          recorder Node -dT $dt -file $dataDir/floorDispCLine1$damp$motion$colType$beamType.out  -time -node $nodeListCLine1 -dof 1 disp;
          recorder Node -dT $dt -file $dataDir/floorVelCLine1$damp$motion$colType$beamType.out  -time -node $nodeListCLine1 -dof 1 vel;
          # recorder Node -dT $dt -file $dataDir/floorDispCLine1Grav$damp$motion$colType$beamType.out  -time -node $nodeListCLine1Grav -dof 1 disp;

          # puts $cmd

          recorder Drift -dT $dt -file $dataDir/floorDrift$damp$motion$colType$beamType.out  -time -iNode $inodeList -jNode $jnodeList  -dof 1 -perpDirn 2 ;
          # puts $cmd

          recorder Node -dT $dt -file $dataDir/NodeMassAccel$damp$motion$colType$beamType.out  -time -node $nodeListMass -dof 1 accel;
          recorder Node -dT $dt -file $dataDir/NodeMassAccelPZ$damp$motion$colType$beamType.out  -time -node $nodeListMassPZ -dof 1 accel;
          recorder Node -dT $dt -file $dataDir/NodeMassVel$damp$motion$colType$beamType.out  -time -node $nodeListMass -dof 1 vel;
          recorder Node -dT $dt -file $dataDir/NodeMassUnbalance$damp$motion$colType$beamType.out  -time -node $nodeListMass -dof 1 unbalance;

          recorder Node -dT $dt -file $dataDir/NodeMassUnbalanceIncInertia$damp$motion$colType$beamType.out  -time -node $nodeListMass -dof 1 unbalanceIncInertia;
          recorder Node -dT $dt -file $dataDir/NodeMassRayleighForces$damp$motion$colType$beamType.out  -time -node $nodeListMass -dof 1 nodalRayleighForces;
          # puts $cmd


          recorder Node -dT $dt -file $dataDir/NodeReaction$damp$motion$colType$beamType.out  -time -node $nodeListReaction -dof 1 reaction;
          recorder Node -dT $dt -file $dataDir/NodeReactionIncInertia$damp$motion$colType$beamType.out  -time -node $nodeListReaction -dof 1 reactionIncInertia;
          recorder Node -dT $dt -file $dataDir/NodeRayleigh$damp$motion$colType$beamType.out  -time -node $nodeListReaction -dof 1 rayleighForces;
          # puts $cmd
          ...
          recorder Element -dT $dt -file $dataDir/EleForcesFloors$damp$motion$colType$beamType.out  -time -ele $colFloorList forces;# puts $cmd
          recorder Element -dT $dt -file $dataDir/EleForcesFloorsGrav$damp$motion$colType$beamType.out  -time -ele $colFloorGravList forces; puts $cmd

          if {$damp == "C"} {
            #	recorder Element -dT $dt -file $dataDir/floorDamperForce$damp$motion$colType$beamType.out  -time -ele $FloorDamperList localForce;
            recorder Element -dT $dt -file $dataDir/floorDamperForce$damp$motion$colType$beamType.out  -time -ele $FloorDamperList globalForces;	
            recorder Element -dT $dt -file $dataDir/floorDamperDeformation$damp$motion$colType$beamType.out  -time -ele $FloorDamperList deformation;
          }

          recorder Node -dT $dt -file $dataDir/motionAccel$damp$motion$colType$beamType.out  -time -node 1 -dof 1 accel
        }
        ...
        if {$recordAll == "YES"} {
          recorder EnvelopeNode -file $dataDir/floorDispEnv.out  -node $nodeList -dof 1 disp
          recorder EnvelopeNode -file $dataDir/floorAccEnv.out  -timeSeries 10 -node $nodeList -dof 1 accel;
          recorder Node -dT $dt -file $dataDir/floorDisp$damp$motion$colType$beamType.out  -time -node $nodeList -dof 1 disp;
          recorder Node -dT $dt -file $dataDir/floorVel$damp$motion$colType$beamType.out  -time -node $nodeList -dof 1 vel;
          recorder Node -dT $dt -file $dataDir/floorAccel$damp$motion$colType$beamType.out  -time -node $nodeList -dof 1 accel;
          recorder Node -dT $dt -file $dataDir/$motion.dat  -time -timeSeries 10 -node 11 -dof 1 accel;
          recorder Node -dT $dt -file $dataDir/floorRotVel$damp$motion$colType$beamType.out  -time -node $nodeList -dof 3 vel;
          recorder Node -dT $dt -file $dataDir/floorReaction$damp$motion$colType$beamType.out  -time -node $nodeList -dof 3 reaction;

          recorder Node -dT $dt -file $dataDir/floorRayleigh$damp$motion$colType$beamType.out  -time -node $nodeList  -dof 3 rayleighForces;

          recorder Node -dT $dt -file $dataDir/floorRayleigh1$damp$motion$colType$beamType.out  -time -node $nodeList  -dof 1 rayleighForces;


          recorder Element -dT $dt -file $dataDir/colEleForces$damp$motion$colType$beamType.out  -time -ele $eleList forces;

          recorder Element -dT $dt -file $dataDir/PZrigidEleForces$damp$motion$colType$beamType.out  -time -ele $PZrigidList forces;

          # add if having panel zone element
          recorder Element -dT $dt -file $dataDir/PZhingeForce$damp$motion$colType$beamType.out  -time -ele $PZspringList force -dir 6;
          recorder Element -dT $dt -file $dataDir/PZhingeDeformation$damp$motion$colType$beamType.out  -time -ele $PZspringList deformation;

          if {$beamType == "Force" || $beamType == "Displacement"} {
            recorder Element -dT $dt -file $dataDir/hingeStressStrain$damp$motion$colType$beamType.out  -time -ele $eleListHinge section forceAndDeformation ;
            recorder Element -dT $dt -file $dataDir/hingeStrain$damp$motion$colType$beamType.out  -time -ele $eleListHinge section deformation ;
            recorder Element -dT $dt -file $dataDir/hingeDefo$damp$motion$colType$beamType.out  -time -ele $eleListHinge basicDeformation ;
            recorder Element -dT $dt -file $dataDir/hingePlasticDefo$damp$motion$colType$beamType.out  -time -ele $eleListHinge plasticDeformation ;
          } elseif {$beamType == "ComponentElement" } {
            recorder Element -dT $dt -file $dataDir/hingeStressStrain$damp$motion$colType$beamType.out  -time -ele $eleListHinge hingeDefoAndForce ;
            recorder Element -dT $dt -file $dataDir/hingeTangent$damp$motion$colType$beamType.out  -time -ele $eleListHinge hingeTangent ;

          } else {
            recorder Element -dT $dt -file $dataDir/hingeTangent$damp$motion$colType$beamType.out  -time -ele $eleListHinge material 1 tangent ;
            recorder Element -dT $dt -file $dataDir/hingeDampingForces$damp$motion$colType$beamType.out  -time -ele $eleListHinge dampingForces ;
            recorder Element -dT $dt -file $dataDir/hingeStressStrain$damp$motion$colType$beamType.out  -time -ele $eleListHinge defoANDforce ;
            recorder Element -dT $dt -file $dataDir/hingePlasticStrain$damp$motion$colType$beamType.out  -time -ele $eleListHinge plasticStrain ;
          }

          if {$colType == "Force" || $colType == "Displacement"} {
            ;
          } else {
            recorder Element -dT $dt -file $dataDir/colHingeStressStrain$damp$motion$colType$beamType.out  -time -ele $eleColListHinge hingeDefoAndForce ; #puts $cmd;
            recorder Element -dT $dt -file $dataDir/colPlasticStrain$damp$motion$colType$beamType.out  -time -ele $eleColListHinge plasticStrain ;
            recorder Element -dT $dt -file $dataDir/colHingeTangent$damp$motion$colType$beamType.out  -time -ele $eleColListHinge hingeTangent ; #puts $cmd;
          }
        }
...
#         progress update
          ...
          if {$ok != 0} {
            puts "\n\n\n NORM ENERGY \n\n\n"
            test EnergyIncr 1.0e-14 20 $testPrint
