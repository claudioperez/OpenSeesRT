...
// command: designVariable  1 -name E  < -startPt $E   -lowerBound [expr $E*0.8]
// -upperBound [expr $E*1.2] >
                                              ...
                                              TCL_Char ** const argv)
{
  ...
  while (argc > argvCounter) {
    if ((strcmp(argv[argvCounter], "-name") == 0) ||
        (strcmp(argv[argvCounter], "-Name") == 0)) {
      argvCounter++;
  ...
  if (theDesignVariable == 0) {
    opserr << "ERROR: could not create random theDesignVariable number " << tag
           << endln;
    return TCL_ERROR;
  }
  ...
  if (theOptimizationDomain->addDesignVariable(theDesignVariable) == false) {
    opserr << "ERROR: failed to add theDesignVariable variable to the domain "
                                                        ...
                                                        TCL_Char ** const argv)
{
  ...
  if (strcmp(argv[argvCounter], "-dvNum") == 0) {
    ...
    if (theDesignVariable == 0) {
      opserr << "ERROR:: A non-existing design variable number " << dvNumber
             << " is being positioned in the model " << endln;
      return TCL_ERROR;
    }
  } else {
  ...
  if (strcmp(argv[argvCounter], "-element") == 0) {
    ...
    theDesignVariablePositioner =
        new DesignVariablePositioner(tag, theOptimizationDomain, dvNumber,
                                     theObject, data, argc - argvCounter);
  ...
  // IF UNCERTAIN *LOAD*
    ...
    theObject =
        (DomainComponent *)theStructuralDomain->getLoadPattern(tagOfObject);
  ...
  // IF UNCERTAIN *NODE* PROPERTY
    ...
    if (theParameter == 0) {
      ...
      theDesignVariablePositioner = new DesignVariablePositioner(
          tag, theOptimizationDomain, dvNumber, *theParameter);
    }
  } else {
  ...
  if (theOptimizationDomain->addDesignVariablePositioner(
          theDesignVariablePositioner) == false) {
    opserr << "ERROR: failed to add random variable positioner number " << tag
           << " to the domain." << endln;
...
// command: objectiveFunction 1  -name F  -GradientName G -tclFile objective.tcl
// ; # -lowerBound -1.e20 -upperBound 1.e20  - multiplier $E -state 0 -linearAdd
// A ;
int
                                                 ...
                                                 TCL_Char ** const argv)
{
  ...
  if (numberOfArguments < 7) {
    opserr << "ERROR: invalid number of arguments to designVariable command "
           << endln;
    opserr << "command: objectiveFunction 1  -name F  -GradientName G  "
              "-tclFile objective.tcl  ; # -lowerBound -1.e20 -upperBound "
              "1.e20 - multiplier $E -state 0 -linearAdd A"
           << endln;
    return TCL_ERROR;
  }
  ...
  while (argc > argvCounter) {
    if ((strcmp(argv[argvCounter], "-name") == 0) ||
    ...
    else if ((strcmp(argv[argvCounter], "-GradientName") == 0) ||
             (strcmp(argv[argvCounter], "-gradientName") == 0)) {
    ...
    else if ((strcmp(argv[argvCounter], "-tclFile") == 0) ||
             (strcmp(argv[argvCounter], "-TclFile") == 0)) {
      argvCounter++;
      ...
      if (Tcl_GetVar2(interp, linearAddName, "1", TCL_GLOBAL_ONLY) != NULL) {
        ...
        opserr << "warning: the linearAdd with name " << linearAddName
               << " does not exit" << endln;
      } // else
  ...
  // ADD THE OBJECT TO THE DOMAIN
  if (theOptimizationDomain->addObjectiveFunction(theObjectiveFunction) ==
      false) {
    opserr << "ERROR: failed to add theObjectiveFunction  to the domain (wrong "
              "number of arguments?)\n";
                                                  ...
                                                  TCL_Char ** const argv)
{
  ...
  if (numberOfArguments < 6) {
    opserr << "ERROR: invalid number of arguments to designVariable command "
           << endln;
    opserr << "command: constraintFunction 1  -name F1  -GradientName G1 "
              "-lowerBound  lBound1 -upperBound uBound1 -tclFile "
              "constraint1.tcl -multiplier M1 -state S1 -linearAdd A1 "
           << endln;
    return TCL_ERROR;
  }
  ...
  while (argc > argvCounter) {
    if ((strcmp(argv[argvCounter], "-name") == 0) ||
      ...
      if (numberOfConstraints == 0) {
        opserr << "Fatal: lowerBound error!" << endln;
        exit(-1);
      }
      //			int numOfDV = theStructuralDomain
      ...
      numberOfConstraints = ii - 1;
      if (numberOfConstraints == 0) {
        opserr << "Fatal: upperBound error!" << endln;
        exit(-1);
      }
      //			int numOfDV = theStructuralDomain
      //->getNumberOfDesignVariables();
      ...
      numberOfConstraints = ii - 1;
      if (numberOfConstraints == 0) {
        opserr << "Fatal: state error!" << endln;
        exit(-1);
      }
      //			int numOfDV = theStructuralDomain
      //->getNumberOfDesignVariables();
      ...
      numberOfConstraints = ii - 1;
      if (numberOfConstraints == 0) {
        opserr << "Fatal: lmultiplier error!" << endln;
        exit(-1);
      }
      //			int numOfDV = theStructuralDomain
      //->getNumberOfDesignVariables();
      ...
      numberOfConstraints = ii - 1;
      if (numberOfConstraints == 0) {
        opserr << "Fatal: linearAdd error!" << endln;
        exit(-1);
      }
  ...
  // ADD THE OBJECT TO THE DOMAIN
  if (theOptimizationDomain->addConstraintFunction(theConstraintFunction) ==
      false) {
    opserr << "ERROR: failed to add theConstraintFunction  to the domain "
              "(wrong number of arguments?)\n";
...
// command: runSNOPTAnalysis -maxNumIter 100 -printOptPointX OptX.out
                                             ...
                                             TCL_Char ** const argv)
{
  ...
  if (numberOfArguments < 3) {
    opserr << "ERROR: invalid number of arguments to designVariable command "
           << endln;
    opserr << "command: runSNOPTAnalysis -maxNumIter 100 -printOptPointX "
              "OptX.out -tclFileToRun tclFileToRun.tcl"
           << endln;
    return TCL_ERROR;
  }
  ...
  while (argc > argvCounter) {
    if ((strcmp(argv[argvCounter], "-maxNumIter") == 0) ||
        (strcmp(argv[argvCounter], "-maxnumiter") == 0)) {
    ...
    else if ((strcmp(argv[argvCounter], "-printFlag") == 0) ||
    ...
    else if ((strcmp(argv[argvCounter], "-printOptPointX") == 0) ||
             (strcmp(argv[argvCounter], "-printoptpointx") == 0)) {
