...
// clientData, Tcl_Interp *interp, int argc, TCL_Char ** const argv);
...
// *interp, int argc, TCL_Char ** const argv);
...
*interp, int argc, TCL_Char ** const argv); int
TclReliabilityModelBuilder_addInitialStaticAnalysis(ClientData clientData,
Tcl_Interp *interp, int argc, TCL_Char ** const argv); int
...
*/
  ...
  // set defaults
  if (theProbabilityTransformation == 0) {
    opserr << "No probabilityTransformation specified, assuming AllIndependent"
           << endln;
    theProbabilityTransformation =
        new AllIndependentTransformation(theReliabilityDomain, 0);
  }

  // reliabilityConvergenceCheck  Standard         -e1 1.0e-3    -e2 1.0e-3
  // -print 1 functionEvaluator            Tcl gradientEvaluator FiniteDifference
  // -pert 1000
  ...
  // meritFunctionCheck           AdkZhang         -multi 2.0    -add 10.0
  // -factor 0.5 stepSizeRule                 Armijo           -maxNum 5    -base
  // 0.5   -initial 1.0 2  -print 0 startPoint                   Mean
                                             ...
                                             TCL_Char ** const argv)
{
  ...
  if (strcmp(argv[2], "normal") == 0) {
    ...
    if (param_indx > 0)
      ...
      opserr << "Rayleigh random variable with tag " << tag
             << " cannot be created with only mean/stdv." << endln;
      return TCL_ERROR;
    }
  }
    ...
    // note userdefined is a special case and will not have any input read from
    // the command line yet unless user defined mean and standard deviation for
    // some reason, which will break this input because we assume argi starts at
    // 3 here
    ...
    if (strcmp(argv[3], "-list") == 0) {
      ...
      for (int i = 0; i < numPoints; ++i) {
        ...
        if (i > 0 && x <= x_old) {
          opserr << "ERROR: x-points to user-defined random variable must be "
                    "consecutive!"
                 << endln;
          return TCL_ERROR;
        }
      ...
      opserr
          << "ERROR: Invalid argument to user-defined random variable, number "
          << tag << endln;
      return TCL_ERROR;
    }
    ...
    opserr << "ERROR: unknown random variable type: " << argv[2]
           << " provided. Must be one of " << endln;
    return TCL_ERROR;
  }
  ...
  if (theReliabilityDomain->addRandomVariable(theRandomVariable) == false) {
    opserr << "ERROR: failed to add random variable to the domain (wrong "
              "number of arguments?)\n";
                                        ...
                                        TCL_Char ** const argv)
{
  ...
  theCorrelationCoefficient =
      new CorrelationCoefficient(tag + 1, rv1, rv2, correlationValue);
                                                ...
                                                TCL_Char ** const argv)
{
  ...
  if (strcmp(argv[1], "homogeneous1") == 0) {
    ...
    if (numEntries != lastRV - firstRV + 1) {
      opserr
          << "WARNING: The number of entries in the correlationVector.txt file "
          << endln
          << " is not equal to the number of random variables that are being "
             "correlated."
          << endln;
    }
                                         ...
                                         TCL_Char ** const argv)
{
  ...
  if (theFunctionEvaluator != 0 && argc > 2) {
    opserr << "ERROR: A limit-state function should not be created after the "
              "GFunEvaluator has been instantiated."
           << endln;
    return TCL_ERROR;
  }
  ...
  // ADD THE OBJECT TO THE DOMAIN
  if (theReliabilityDomain->addLimitStateFunction(theLimitStateFunction) ==
      false) {
                                             ...
                                             TCL_Char ** const argv)
{
  ...
  if (theFunctionEvaluator != 0) {
    // opserr << "ERROR: A limit-state function should not be created after the
    // FunctionEvaluator has been instantiated." << endln; return TCL_ERROR;
  }
                                                 ...
                                                 TCL_Char ** const argv)
{
  ...
  if (strcmp(argv[2], "gamma") == 0) {
    ...
    if (argc != 7) {
      opserr
          << "ERROR: Incorrect number of arguments to gamma modulating function"
          << endln;
      return TCL_ERROR;
    }
    ...
    if (theFilter == 0) {
      opserr << "ERROR: Could not find the filter with tag " << filterTag
             << endln;
      return TCL_ERROR;
    }
    ...
    // CREATE THE OBJECT
    theModulatingFunction =
        new GammaModulatingFunction(thisTag, theFilter, a, b, c);
    ...
    // ADD THE OBJECT TO THE DOMAIN
    if (theReliabilityDomain->addModulatingFunction(theModulatingFunction) ==
        false) {
    ...
    // CREATE THE OBJECT
    theModulatingFunction =
        new ConstantModulatingFunction(thisTag, theFilter, amplitude);
    ...
    // CREATE THE OBJECT
    theModulatingFunction = new TrapezoidalModulatingFunction(
        thisTag, theFilter, t1, t2, t3, t4, amplitude);
    ...
    // CREATE THE OBJECT
    theModulatingFunction =
        new KooModulatingFunction(thisTag, theFilter, t1, t2);
                                     ...
                                     int argc, TCL_Char ** const argv)
{
  ...
  if ((strcmp(argv[2], "standard") == 0) ||
      (strcmp(argv[2], "standardDisplacement") == 0) ||
      (strcmp(argv[2], "Koo") == 0) ||
      (strcmp(argv[2], "standardVelocity") == 0) ||
    ...
    if ((strcmp(argv[2], "standard") == 0) ||
        (strcmp(argv[2], "standardDisplacement") == 0)) {
      theFilter = new StandardLinearOscillatorDisplacementFilter(tag, period_Tn,
                                                                 damping);
    } else if (strcmp(argv[2], "Koo") == 0) {
      theFilter = new KooFilter(tag, period_Tn, damping);
    } else if (strcmp(argv[2], "standardVelocity") == 0) {
      theFilter =
          new StandardLinearOscillatorVelocityFilter(tag, period_Tn, damping);
    } else if (strcmp(argv[2], "standardAcceleration") == 0) {
      theFilter = new StandardLinearOscillatorAccelerationFilter(tag, period_Tn,
                                                                 damping);
    } else {
    ...
    /*
            ...
            if ( strcmp(argv[2],"whitenoise") == 0){
                    ...
                    theFilter = new
    NewStandardLinearOscillatorAccelerationFilter (tag, period_Tn, damping,
    dtpulse);
            }
            */
  } else {
        ...
        if ( (strcmp(argv[2],"delta") == 0) || (strcmp(argv[2],"Delta") == 0) )
{
                        ...
                        if (theFilter == 0) {
                                opserr << "ERROR: ran out of memory creating
filter \n"; opserr << "filter: " << tag << endln; return TCL_ERROR;
                        ...
                        if (theReliabilityDomain->addFilter(theFilter) == false)
{ opserr << "ERROR: failed to add filter to the domain\n"; opserr << "filter: "
<< tag << endln; delete theFilter; // otherwise memory leak return TCL_ERROR;
                        }
...
(strcmp(argv[2],"standardDisplacement") == 0) ) {
                ...
                theFilter = new
StandardLinearOscillatorDisplacementFilter(tag,period_Tn,damping);
        }
        else if ( strcmp(argv[2],"Koo") == 0 ) {
                ...
                theFilter = new
StandardLinearOscillatorVelocityFilter(tag,period_Tn,damping);
        }
        else if (strcmp(argv[2],"standardAcceleration") == 0) {

                theFilter = new
StandardLinearOscillatorAccelerationFilter(tag,period_Tn,damping);
        }
        else {
                opserr << "ERROR:: Unknown type of filter. " << endln;
                return TCL_ERROR;
                                       ...
                                       TCL_Char ** const argv)
{
  ...
  if (strcmp(argv[2], "jonswap") == 0) {
    ...
    if (fmod((argc - 3), 2.0)) {
      opserr << "ERROR: Inconsistent number of points given to spectrum " << tag
             << endln;
      return TCL_ERROR;
    }
                                                        ...
                                                        TCL_Char ** const argv)
{
  ...
  if (argc != 2 && argc != 4) {
    opserr << "ERROR: Wrong number of arguments to probability transformation."
           << endln;
    return TCL_ERROR;
  }
  ...
  if (strcmp(argv[1], "Nataf") == 0) {
    ...
    theProbabilityTransformation =
        new NatafProbabilityTransformation(theReliabilityDomain, printFlag);
  }
    ...
    theProbabilityTransformation =
        new AllIndependentTransformation(theReliabilityDomain, printFlag);
  }
                                              ...
                                              TCL_Char ** const argv)
{
  ...
  if (strcmp(argv[1], "iHLRF") == 0) {
    ...
    if (argc != 2) {
      opserr << "ERROR: Wrong number of arguments to iHLRF search direction. "
             << endln;
      return TCL_ERROR;
    }
    ...
    if (argc != 2) {
      opserr << "ERROR: Wrong number of arguments to GradientProjection search "
                "direction. "
             << endln;
      return TCL_ERROR;
    }
    ...
    if (theStepSizeRule == 0) {
      opserr << "Need theStepSizeRule before a "
                "GradientProjectionSearchDirection can be created"
             << endln;
      return TCL_ERROR;
    }
    ...
    theSearchDirection = new GradientProjectionSearchDirection(
        theStepSizeRule, theProbabilityTransformation, theFunctionEvaluator,
    ...
    // KRM 5-19-2012 all these mixed classes are going to need to change,
    // hessian needs to go in hessian, merit function needs to go in merit
    // function, etc.
                                               ...
                                               TCL_Char ** const argv)
{
  ...
  if (strcmp(argv[1], "FiniteDifference") == 0) {
    ...
    if (theFunctionEvaluator == 0) {
      opserr << "Need FunctionEvaluator before a FiniteDifferenceHessian can "
                "be created"
             << endln;
      return TCL_ERROR;
    }
    ...
    if (argc > 2) {
      ...
      if (argc == 3 || argc == 4) {
        ...
        opserr
            << "ERROR: Wrong number of arguments to FiniteDifferenceGradient. "
            << endln;
        return TCL_ERROR;
      }
      ...
      for (int i = 1; i <= numExtras; ++i) {
        ...
        if (strcmp(argv[counter], "-pert") == 0) {
          ...
          opserr << "ERROR: Error in input to FiniteDifferenceGradient. "
                 << endln;
          return TCL_ERROR;
        }
      }
    ...
    theHessianEvaluator = new FiniteDifferenceHessian(
        theFunctionEvaluator, theReliabilityDomain, theStructuralDomain);
  }
    ...
    if (theSQPtriplePurpose == 0) {
      opserr << "Need theSQPSearchDirection before a SQP Hessian Approximation "
                "can be created"
             << endln;
      return TCL_ERROR;
    }
    ...
    // Set the Hessian approximation in the search direction
    // (this needs to be changed for generatlity; new method of search
    // direction)
    // theSQPtriplePurpose->setHessianApproximation(theHessianEvaluator);
  ...
  if (strcmp(argv[argvCounter], "AdkZhang") == 0) {
    ...
    while (argvCounter < argc) {
      ...
      if (strcmp(argv[argvCounter], "-multi") == 0) {
        ...
        opserr << "ERROR: Invalid input to AdkZhang merit function check. "
               << endln;
        return TCL_ERROR;
      }
    }
    ...
    if (multi < 1.0 || add < 0.0) {
      opserr << "ERROR: Invalid values of multi/add parameters to AdkZhang "
    ...
    if (thePolakHeDualPurpose == 0) {
      opserr << "Need thePolakHeSearchDirection before a PolakHe merit "
    ...
    while (argvCounter < argc) {
      ...
      if (strcmp(argv[argvCounter], "-factor") == 0) {
        ...
        opserr << "ERROR: Invalid input to Polak He merit function check. "
               << endln;
        return TCL_ERROR;
      }
    }
    ...
    // Check that the SQP search direction is already created
    if (theSQPtriplePurpose == 0) {
      opserr << "Need theSQPSearchDirection before a SQP merit function can be "
                "created"
             << endln;
      return TCL_ERROR;
    }
                                                          ...
                                                          TCL_Char ** const argv)
{
  ...
  if (argc < 2) {
    opserr
        << "ERROR: Wrong number of arguments to reliability convergence check."
        << endln;
    return TCL_ERROR;
  }
  ...
  if (strcmp(argv[argvCounter], "Standard") == 0) {
    ...
    while (argvCounter < argc) {
      ...
      if (strcmp(argv[argvCounter], "-e1") == 0) {
        ...
        opserr << "ERROR: Invalid input to standard reliability convergence "
                  "check. "
               << endln;
        return TCL_ERROR;
      }
    ...
    theReliabilityConvergenceCheck =
        new StandardReliabilityConvergenceCheck(e1, e2, scaleValue, print);
  }
    ...
    theReliabilityConvergenceCheck =
        new OptimalityConditionReliabilityConvergenceCheck(e1, e2, scaleValue,
                                                           print);
  } else {
                                           ...
                                           TCL_Char ** const argv)
{
  ...
  if (strcmp(argv[argvCounter], "Armijo") == 0) {
    ...
    if (theFunctionEvaluator == 0) {
      opserr
          << "Need theGFunEvaluator before an ArmijoStepSizeRule can be created"
          << endln;
      return TCL_ERROR;
    }
    if (theProbabilityTransformation == 0) {
    ...
    if (theMeritFunctionCheck == 0) {
      opserr << "Need theMeritFunctionCheck before a ArmijoStepSizeRule can be "
                "created"
             << endln;
      return TCL_ERROR;
    }
    ...
    theStepSizeRule = new ArmijoStepSizeRule(
        theReliabilityDomain, theFunctionEvaluator,
        theProbabilityTransformation, theMeritFunctionCheck,
        theRootFindingAlgorithm, base, maxNumReductions, b0, numberOfShortSteps,
        radius, surfaceDistance, evolution, printFlag);
                                                ...
                                                TCL_Char ** const argv)
{
  ...
  if (strcmp(argv[1], "Matlab") == 0) {
    opserr << "ERROR: The Matlab function evaluator is not implemented in this "
           << endln
           << " version of your OpenSees executable file. Please contact the "
           << endln << " developer for more information." << endln;
    return TCL_ERROR;
  } else if (strcmp(argv[1], "Tcl") == 0) {
    if (argc == 2)
      theFunctionEvaluator =
          new TclEvaluator(interp, theReliabilityDomain, theStructuralDomain);

    else if (argc == 4) {
      if (strcmp(argv[2], "-file") != 0 && strcmp(argv[2], "-command") != 0) {
        opserr << "ERROR: Tcl function evaluator only takes -file and -command "
                  "arguments."
               << endln;
        return TCL_ERROR;
      } else {
  ...
  /*
          ...
          //theFunctionEvaluator = new AnalyzerGFunEvaluator(interp,
                                                ...
                                                TCL_Char ** const argv)
{
  ...
  if (strcmp(argv[1], "FiniteDifference") == 0) {
    ...
    // Check that the necessary ingredients are present
    if (theFunctionEvaluator == 0) {
      opserr << "Need FunctionEvaluator before a FiniteDifferenceGradient can "
                "be created"
             << endln;
      return TCL_ERROR;
    }
    ...
    theGradientEvaluator = new FiniteDifferenceGradient(
        theFunctionEvaluator, theReliabilityDomain, theStructuralDomain);
  }

  else if (strcmp(argv[1], "OpenSees") == 0 ||
           strcmp(argv[1], "Implicit") == 0) {

    bool doGradientCheck = false;

    // Quan Apr. 2006
    //	if (theSensitivityAlgorithm == 0) {
    //		opserr << "Warning:Need a DDM sensitivity algorithm before a OpenSees
    //sensitivity evaluator can be created" << endln; 		return TCL_ERROR;
    //	}
    ...
    theGradientEvaluator =
        new ImplicitGradient(theFunctionEvaluator, theReliabilityDomain,
                             theStructuralDomain, theSensitivityAlgorithm);
  ...
  /*
          ...
          if (theAnalyzer == 0) {
                  opserr << "Need Analyzer before a Analyzer sensitivity
  evaluator can be created" << endln; return TCL_ERROR;
          }
          if (theSensitivityAlgorithm == 0) {
                  opserr << "Need a DDM sensitivity algorithm before a Analyzer
  sensitivity evaluator can be created" << endln; return TCL_ERROR;
          }
          ...
          if (argc==2) {
                  ...
                  opserr << "ERROR: Wrong number of arguments to gradGEvaluator.
  " << endln; return TCL_ERROR;
          }

          theGradientEvaluator = new AnalyzerGradGEvaluator(interp,
                                             ...
                                             TCL_Char ** const argv)
{
  ...
  if (theFunctionEvaluator == 0) {
    opserr << "Need theFunctionEvaluator before a findCurvatures can be created"
           << endln;
    return TCL_ERROR;
  }
  if (theFORMAnalysis == 0) {
    opserr << "Need theFORMAnalysis before a findCurvatures can be created"
           << endln;
    return TCL_ERROR;
  }
  ...
  if (strcmp(argv[1], "firstPrincipal") == 0) {
    // KRM 5-22-2012
    // combining former firstPrincipal and bySearchAlgorithm because they are
    // doing same thing
    theFindCurvatures = new FirstPrincipalCurvature(
        theReliabilityDomain, theFunctionEvaluator, theFORMAnalysis);
    ...
    theFindCurvatures = new CurvaturesBySearchAlgorithm(
        theReliabilityDomain, theFunctionEvaluator, theFORMAnalysis,
        numberOfCurvatures);
    ...
    if (theHessianEvaluator == 0) {
      opserr
          << "Need theHessianEvaluator before curvatureFitting can be created"
          << endln;
      return TCL_ERROR;
    }
    ...
    if (theProbabilityTransformation == 0) {
      opserr << "Need theProbabilityTransformation before curvatureFitting can "
                "be created"
             << endln;
      return TCL_ERROR;
    }

    theFindCurvatures = new CurvatureFitting(
        theReliabilityDomain, theStructuralDomain, theFunctionEvaluator,
                                                       ...
                                                       TCL_Char ** const argv)
{
  ...
  if (argc < 2) {
    opserr << "ERROR: Wrong number of arguments to find design point algorithm."
           << endln;
    return TCL_ERROR;
  }
  ...
  if (strcmp(argv[argvCounter], "StepSearch") == 0) {
    ...
    // Check that the necessary ingredients are present
    if (theFunctionEvaluator == 0) {
      opserr << "Need theGFunEvaluator before a FindDesignPointAlgorithm can "
                "be created"
             << endln;
      return TCL_ERROR;
    }
    if (theGradientEvaluator == 0) {
      opserr << "Need theGradientEvaluator before a FindDesignPointAlgorithm "
                "can be created"
             << endln;
      return TCL_ERROR;
    }
    if (theStepSizeRule == 0) {
      opserr << "Need theStepSizeRule before a FindDesignPointAlgorithm can be "
                "created"
             << endln;
      return TCL_ERROR;
    }
    if (theSearchDirection == 0) {
      opserr << "Need theSearchDirection before a FindDesignPointAlgorithm can "
                "be created"
             << endln;
      return TCL_ERROR;
    }
    ...
    theFindDesignPointAlgorithm = new SearchWithStepSizeAndStepDirection(
        ...
        theFunctionEvaluator, theGradientEvaluator, theStepSizeRule,
    ...
    */
    ...
    opserr
        << "ERROR: unrecognized type of FindDesignPointAlgorithm Algorithm \n";
    return TCL_ERROR;
  }
                                         ...
                                         TCL_Char ** const argv)
{
  ...
  if (strcmp(argv[1], "Mean") == 0) {
    ...
    if (inputFile.fail()) {
      opserr << "File " << argv[2] << " could not be opened for startPoint. "
             << endln;
      return TCL_ERROR;
    }
    ...
    if (numEntries != nrv) {
      opserr << "ERROR: Wrong number of entries in the file read by startPoint."
             << endln;
      return TCL_ERROR;
    }
                                          ...
                                          TCL_Char ** const argv)
{
  ...
  if (theFunctionEvaluator == 0) {
    opserr << "Need theGFunEvaluator before a root-finding algorithm can be "
              "created"
           << endln;
    return TCL_ERROR;
  }
  ...
  if (theProbabilityTransformation == 0) {
    opserr << "Need theProbabilityTransformation before a root-finding "
  ...
  if (strcmp(argv[1], "Secant") == 0) {
    ...
    theRootFindingAlgorithm = new SecantRootFinding(
        theReliabilityDomain, theProbabilityTransformation,
        theFunctionEvaluator, maxIter, tol, maxStepLength);
                                           ...
                                           TCL_Char ** const argv)
{
  ...
  // Check number of arguments
  if ((argc != 2) && (argc != 4)) {
    opserr << "ERROR: Wrong number of input parameter to FORM analysis"
           << endln;
    return TCL_ERROR;
  }
  ...
  if (theFindDesignPointAlgorithm == 0) {
    opserr << "Need theFindDesignPointAlgorithm before a FORMAnalysis can be "
              "created"
           << endln;
    return TCL_ERROR;
  }
  if (theFunctionEvaluator == 0) {
    opserr << "Need theFunctionEvaluator before a FORMAnalysis can be created"
           << endln;
    return TCL_ERROR;
  }
  if (theProbabilityTransformation == 0) {
    opserr << "Need theProbabilityTransformation before a FORMAnalysis can be "
              "created"
           << endln;
    return TCL_ERROR;
  }
  ...
  theFORMAnalysis = new FORMAnalysis(
      theReliabilityDomain, theFindDesignPointAlgorithm, theFunctionEvaluator,
      theProbabilityTransformation, argv[1], relSensTag);
                                           ...
                                           TCL_Char ** const argv)
{
  ...
  // Check number of arguments
  if (argc != 2) {
    opserr << "ERROR: Wrong number of input parameter to FOSM analysis"
           << endln;
    return TCL_ERROR;
  }
  ...
  if (theFunctionEvaluator == 0) {
    opserr << "Need theGFunEvaluator before a FOSMAnalysis can be created"
           << endln;
    return TCL_ERROR;
  }
  if (theGradientEvaluator == 0) {
    opserr << "Need theGradientEvaluator before a FOSMAnalysis can be created"
           << endln;
    return TCL_ERROR;
  }
...
clientData, Tcl_Interp *interp, int argc, TCL_Char ** const argv)
{
        ...
        // Check number of arguments
        if (argc != 9)  {
                opserr << "ERROR: Wrong number of input parameter to Fragility
analysis" << endln; return TCL_ERROR;
        }
...
ParametricReliabilityAnalysis can be created" << endln; return TCL_ERROR;
        }
        if (theGradientEvaluator == 0 ) {
...
TCL_ERROR;
                        ...
                        opserr << "ERROR: invalid input to Fragility analysis "
<< endln; return TCL_ERROR;
                }
        ...
        if (parGiven && rangeGiven && numIntGiven) {
                theParametricReliabilityAnalysis = new
ParametricReliabilityAnalysis( theReliabilityDomain,
                ...
                opserr << "ERROR:: some input to
theParametricReliabilityAnalysis was not provided" << endln; return TCL_ERROR;
        }

        if (theParametricReliabilityAnalysis == 0) {
                opserr << "ERROR: could not create
theParametricReliabilityAnalysis \n"; return TCL_ERROR;
        }
                                           ...
                                           TCL_Char ** const argv)
{
  ...
  // check for essential ingredients
  if (theFindCurvatures == 0) {
    opserr << "Need theFindCurvatures before a SORMAnalysis can be created"
           << endln;
    return TCL_ERROR;
  }
  if (theFORMAnalysis == 0) {
    opserr << "ERROR: The current SORM implementation requires a FORM analysis"
           << endln << " to have been executed previously in the same session."
           << endln;
    return TCL_ERROR;
  }
                                             ...
                                             TCL_Char ** const argv)
{
  ...
  // Check for essential ingredients
  if (theFunctionEvaluator == 0) {
    opserr << "Need theFunctionEvaluator before a SystemAnalysis can be created"
           << endln;
    return TCL_ERROR;
  }
  ...
  if (argc < 4 || argc % 2 > 0) {
    opserr << "ERROR: Wrong number of arguments to System Reliability analysis"
           << endln;
    opserr
        << "Want: runSystemAnalysis fileName? analysisMethod? (allInParallel | "
           "allInSeries) <-Nmax val?> <-tol val?> <B_fileName R_fileName>"
        << endln;
    ...
    while (argi < argc) {
      if (strcmp(argv[argi], "-Nmax") == 0) {
        ...
        // option of specifying files with beta and rho instead of using
        // information in the reliability domain
  ...
  if (strcmp(argv[3], "allInParallel") == 0)
    ...
    opserr << "ERROR: Invalid system reliability analysis type input:"
           << argv[3] << endln;
    return TCL_ERROR;
  }
  ...
  if (strcmp(argv[2], "PCM") == 0)
    ...
    theSystemAnalysis =
        new MVNcdf(theReliabilityDomain, theFunctionEvaluator, argv[1], aType,
                   betaFile, rhoFile, nMax, tol);
                                                         ...
                                                         TCL_Char ** const argv)
{
  ...
  // Check for essential tools
  if (theProbabilityTransformation == 0) {
    opserr << "Need theProbabilityTransformation before a SimulationAnalyis "
              "can be created"
           << endln;
    return TCL_ERROR;
  }
  if (theFunctionEvaluator == 0) {
    opserr << "Need theGFunEvaluator before a SimulationAnalyis can be created"
           << endln;
    return TCL_ERROR;
  }
  if (theRandomNumberGenerator == 0) {
    opserr << "Need theRandomNumberGenerator before a SimulationAnalyis can be "
              "created"
           << endln;
    return TCL_ERROR;
  }
  ...
  if (analysisTypeTag == 2 && printFlag == 2) {
    opserr << "ERROR:: The restart option of the sampling analysis cannot be "
           << endln << " used together with the response statistics option. "
           << endln;
    return TCL_ERROR;
  }
...
// command "runOutCrossingAnalysis  filename?  -results stepsToStart?
// stepsToEnd?  samplefreq? impulseFreq?   -littleDt dt? -analysisType option
// for analysisType 1:   -twoSearches   <-integralTolerance  tol?
// -useFirstDesignPoint>
//            analysisType 2:    -Koo
//
int
                                                  ...
                                                  TCL_Char ** const argv)
{
  ...
  // check for essential ingredients
  if (theFindDesignPointAlgorithm == 0) {
    opserr << "Need theFindDesignPointAlgorithm before an OutCrossingAnalysis "
              "can be created"
           << endln;
    return TCL_ERROR;
  }
  if (theFunctionEvaluator == 0) {
    opserr
        << "Need theGFunEvaluator before an OutCrossingAnalysis can be created"
        << endln;
    return TCL_ERROR;
  }
  if (theGradientEvaluator == 0) {
    opserr << "Need theGradientEvaluator before an OutCrossingAnalysis can be "
              "created"
           << endln;
    return TCL_ERROR;
  }
  ...
  theOutCrossingAnalysis = new OutCrossingAnalysis(
      theReliabilityDomain, theFunctionEvaluator, theGradientEvaluator,
      theFindDesignPointAlgorithm, analysisType, stepsToStart, stepsToEnd,
      sampleFreq, impulseFreq, littleDt, argv[1], integralTolerance,
      useFirstDesignPt);
...
// command "runOrthogonalPlaneSamplingAnalysis  -fileName filename?  -maxNum
// number?   -type  analysisType? -targetCOV cov? -print printFlag? -funcTol
// tol1? -varTol tol2? -maxIter iter? -littleDt littleDt?.... option for
// analysisType  "failureProbability"   ---1:   failure probability.
    ...
    ClientData clientData, Tcl_Interp *interp, int argc, TCL_Char ** const argv)
{
  ...
  if (argc < 3) {
    opserr << "command: runOrthogonalPlaneSamplingAnalysis  -fileName "
              "filename?  -maxNum number?   -type  analysisType? -targetCOV "
              "cov? -print printFlag? ";
  ...
  /*
  if (theStartPoint == 0 ) {
          opserr << "orthogonalPlaneSamplingAnalysis can not run. Need
  StartPoint !" << endln; return TCL_ERROR;
  }
  ...
  */
  ...
  int argvCounter = 1;
  while (argc > argvCounter) {
    if ((strcmp(argv[argvCounter], "-fileName") == 0) ||
        (strcmp(argv[argvCounter], "-filename") == 0)) {
      argvCounter++;
  ...
  /*
     ...
     *passedRandomNumberGenerator, int passedNumberOfSimulations, int
     passedMaxNumOfIterations, double passedTargetCOV, double samplingStdv, int
     printFlag, TCL_Char *fileName, Vector * pDesignPoint, int analysisTypeTag,
                                          int zeroFindingType);
...
// Quan & Michele: add command for visualization of another zerofinding
// algorithm
//  command: runGFunVizAnalysis outputfile -space y    -funSurf surface     -dir
//  file designpoint.out -file filename numPts? -zeroFindingAlgorithm
//  safeguardedZeroFinding
                                                        ...
                                                        TCL_Char ** const argv)
{
  ...
  // check for essential ingredients
  if (theFunctionEvaluator == 0) {
    opserr << "Need theGFunEvaluator before a GFunVisualizationAnalysis can be "
              "created"
           << endln;
    return TCL_ERROR;
  }
  if (theProbabilityTransformation == 0) {
    opserr << "Need theProbabilityTransformation before a "
              "GFunVisualizationAnalysis can be created"
           << endln;
    return TCL_ERROR;
  }
  ...
  while (argc > argvCounter) {
    if (strcmp(argv[argvCounter], "-convResults") == 0) {
      ...
      if ((argc - 1) < argvCounter) {
        opserr << "ERROR: No file name found for visualization of convergence "
                  "results. "
               << endln;
        return TCL_ERROR;
      }
      ...
      if (strcmp(argv[argvCounter], "X") == 0 ||
          strcmp(argv[argvCounter], "x") == 0) {
        space = 1;
      } else if (strcmp(argv[argvCounter], "Y") == 0 ||
                 strcmp(argv[argvCounter], "y") == 0) {
        space = 2;
      } else {
      ...
      if (strcmp(argv[argvCounter], "rv") == 0) {
        ...
        if (inputFile.fail()) {
          opserr << "File " << argv[argvCounter] << " could not be opened. "
                 << endln;
          return TCL_ERROR;
        }
        ...
        if (numEntries != numRVs) {
          opserr << "ERROR: Wrong number of entries in the the file "
                 << argv[argvCounter] << endln;
          return TCL_ERROR;
        }
      ...
      if (numEntries == 0) {
        opserr
            << "ERROR: No entries in the file read by visualization analysis!"
            << endln;
        return TCL_ERROR;
      }
      ...
      opserr << "ERROR: invalid input to theGFunVisualizationAnalysis."
             << endln;
      return TCL_ERROR;
    }
  }
  ...
  // dir         [ 0:(needed for surface), 1:rv,       2:file    ] (pass rvDir
  // or theDirectionVector) axes        [ 0:error,   1:coords1,   2:coords2,
  // 3:file    ] (pass axesVector... or theMatrix+numLinePts)

  if (space == 0 || funSurf == 0 || axes == 0) {
    opserr << "ERROR: Some input is missing to the visualization analysis."
           << endln;
    return TCL_ERROR;
  }
  if (dir == 0 && funSurf == 2) {
    opserr
        << "A direction is needed for visualization of the limit-state surface."
        << endln;
    return TCL_ERROR;
  }
  ...
  if (zeroFindingAlg == 0)
    theGFunVisualizationAnalysis = new GFunVisualizationAnalysis(
        theReliabilityDomain, theFunctionEvaluator,
        theProbabilityTransformation, argv[1], argv[convFileArgv], convResults,
        space, funSurf, axes, dir);
  /*	else if (zeroFindingAlg ==1)
             theGFunVisualizationAnalysis = new
     GFunVisualizationSamplingAnalysis( theReliabilityDomain,
                                                                                          theFunctionEvaluator,
                                                                                          theProbabilityTransformation,
                                                                                          argv[1],
                                                                                          argv[convFileArgv],
  ...
  */
  ...
  if (axes == 1 || axes == 2) {
    ...
    //		if (theStartPoint == 0 ) {
    //			opserr << "Need theStartPoint before this GFunVisualizationAnalysis
  ...
  if (convResults == 1) {
    ...
    if (theGradientEvaluator == 0) {
      opserr << "Need theGradientEvaluator before this "
                "GFunVisualizationAnalysis can be created"
             << endln;
      return TCL_ERROR;
    }
    ...
    if (theMeritFunctionCheck == 0) {
      opserr << "Need theMeritFunctionCheck before this "
                "GFunVisualizationAnalysis can be created"
             << endln;
      return TCL_ERROR;
    }
    ...
    if (theReliabilityConvergenceCheck == 0) {
      opserr << "Need theReliabilityConvergenceCheck before this "
                "GFunVisualizationAnalysis can be created"
             << endln;
      return TCL_ERROR;
    }
    ...
    theGFunVisualizationAnalysis->setReliabilityConvergenceCheck(
        theReliabilityConvergenceCheck);
  }
  ...
  if (funSurf == 2) {
    ...
    if (theRootFindingAlgorithm == 0) {
      opserr << "Need theRootFindingAlgorithm before this "
                "GFunVisualizationAnalysis can be created"
             << endln;
      return TCL_ERROR;
    }

    theGFunVisualizationAnalysis->setRootFindingAlgorithm(
        theRootFindingAlgorithm);
  }
...
/// Command:  runMonteCarloResponseAnalysis  -outPutFile  m.out -maxNum 1000
                                                         ...
                                                         TCL_Char ** const argv)
{
  ...
  if (numberOfArguments < 4) {
    opserr << "ERROR: invalid number of arguments to designVariable command "
           << endln;
    opserr << "command: runMonteCarloResponseAnalysis  -outPutFile  m.out "
              "-maxNum 1000 -print 1 -tclFileToRun test.tcl"
           << endln;
    return TCL_ERROR;
  }
  ...
  int argvCounter = 1;
  while (argc > argvCounter) {
    if ((strcmp(argv[argvCounter], "-maxNum") == 0) ||
        (strcmp(argv[argvCounter], "-maxnum") == 0)) {
      ...
      argvCounter++;
             ...
             (strcmp(argv[argvCounter], "-printFlag") == 0)) {
    ...
    else if ((strcmp(argv[argvCounter], "-outPutFile") == 0) ||
             (strcmp(argv[argvCounter], "-outputfile") == 0)) {
    ...
    else if ((strcmp(argv[argvCounter], "-tclFileToRun") == 0) ||
             (strcmp(argv[argvCounter], "-TclFileToRun") == 0)) {
      argvCounter++;
...
///////
///  Command:  updateParameterValue  -rv 1 -value 20.0  or updateParameter
///  -startPoint
                                                ...
                                                TCL_Char ** const argv)
{
  /*
        ...
        int argvCounter = 1;
        while (argc > argvCounter) {
                if ((strcmp(argv[argvCounter],"-dv") ==
0)||(strcmp(argv[argvCounter],"-dvNum") == 0)) {
...
TCL_OK) { opserr << "ERROR: invalid input: dv number \n"; return TCL_ERROR;
                ...
                else if ((strcmp(argv[argvCounter],"-rv") ==
0)||(strcmp(argv[argvCounter],"-rvNum") == 0)) {
...
//			opserr<<"value is:"<<value<<endln;
                ...
                else if ((strcmp(argv[argvCounter],"-startPoint") ==
0)||(strcmp(argv[argvCounter],"-startpoint") == 0)) { argvCounter++;
                        usingStartPt = true;
                }

                else {
                        opserr<<"warning: unknown command: updateparameter"
<<argv[argvCounter]<<endln; argvCounter++;
...
=0; numberOfRVDVPositioners =
theReliabilityDomain->getNumberOfRandomVariablePositioners(); if
(numberOfRVDVPositioners==0) {opserr<<"warnning: updateParameter no
randomVariablePositioner"<<endln;  } else { int rvNumber; for (int i=1 ;
i<=numberOfRVDVPositioners ; i++ )  { theRandomVariablePositioner =
theReliabilityDomain->getRandomVariablePositionerPtr(i); rvNumber =
theRandomVariablePositioner->getRvNumber(); if (rvNumber == dvrv )
...
theReliabilityDomain->getDesignVariablePtr(dvrv);
                        ...
                        RandomVariablePositioner * theRandomVariablePositioner
=0; int numberOfRVDVPositioners =
theReliabilityDomain->getNumberOfRandomVariablePositioners(); for (int i=1 ;
i<=numberOfRVDVPositioners ; i++ )  { theRandomVariablePositioner =
theReliabilityDomain->getRandomVariablePositionerPtr(i); int rvNumber =
theRandomVariablePositioner->getRvNumber(); value =
(*theStartPoint)(rvNumber-1); theRandomVariablePositioner->update(value);
                        }
...
///////
///  Command:  computeHessian -FDM -file $filename1 -designPoint $fileName
///  -perturbation $pTol
//////
int
                                          ...
                                          TCL_Char ** const argv)
{
  ...
  while (argc > argvCounter) {
    ...
    if ((strcmp(argv[argvCounter], "-FDM") == 0) ||
        (strcmp(argv[argvCounter], "-FFD") == 0)) {
      ...
      FDM = true;
             ...
             (strcmp(argv[argvCounter], "-File") == 0)) {
      argvCounter++;
      ...
      argvCounter++;
             ...
             (strcmp(argv[argvCounter], "-designpoint") == 0)) {
      argvCounter++;
  ...
  if (inputFile.good()) {
    ...
    opserr << "ERROR: designpoint can not read from file " << designPointFile
           << endln;
    exit(-1);
  }

  // KRM 5-19-2012
  // this needs to be implemented properly using the reliability base classes
  // (FunctionEvaluator, GradientEvaluator, HessianEvaluator, etc.)
  // Hessian * theHessian = new
  // Hessian(size,theReliabilityDomain,theProbabilityTransformation,theFunctionEvaluator,theGradientEvaluator,pTol);
...
// MultiDimVisualPrinPlane -funSurf function -designPt dp.out  -ndir $n -output
// vis.out <-gridInfo {0  minY  maxY nPts0 1 minX1  maxX1 nPts1 2 minX2  maxX2
// nPts2 ...}  -timeVariant -littleDt 0.001> <-saveHessian $filename>
                                                 ...
                                                 TCL_Char ** const argv)
{
  ...
  while (argc > argvCounter) {
    ...
    if ((strcmp(argv[argvCounter], "-funSurf") == 0) ||
        (strcmp(argv[argvCounter], "-FunSurf") == 0)) {
      argvCounter++;
      if (strcmp(argv[argvCounter], "function") == 0)
        type = 1;
      else if (strcmp(argv[argvCounter], "surface") == 0)
    ...
    else if ((strcmp(argv[argvCounter], "-designPt") == 0) ||
             (strcmp(argv[argvCounter], "-designPoint") == 0)) {
      char fileName[20];
      argvCounter++;
      strcpy(fileName, argv[argvCounter]);
      ...
      if (inputFile.good()) {
        ...
        opserr << "ERROR: designpoint can not read from file " << fileName
               << endln;
        exit(-1);
      }
      ...
      if (inputFile.good()) {
        ...
        while (ii < nrv) {
          for (jj = 0; jj < nrv; jj++) {
            if (!inputFile.eof()) {
              ...
              opserr << "-saveHessian, size of Hessian in file: "
                     << hessianFileName << "is wrong" << endln;
              exit(-1);
            }
          }
        ...
        opserr << "warning: no data in hessianfrom file: " << hessianFileName
               << endln;
      }
  ...
  if (hessianFileName != 0)
    delete hessianFileName;
...
///
// KRM - I see no reason for this code to be in TclReliabilityBuilder, will
// delete soon not being used by any other classes
                                         ...
                                         TCL_Char ** const argv)
{
  ...
  while (argc > argvCounter) {
    if (strcmp(argv[argvCounter], "-fileX") == 0) {
      ...
      opserr << "warning: unknown command: updateparameter" << argv[argvCounter]
             << endln;
      argvCounter++;
  ...
  if (inputFile.good()) {
    ...
    while (ii < nrv) {
      if (!inputFile.eof()) {
        ...
        opserr << "file" << filenameX << " has different size than numRV"
               << endln;
        exit(-1);
      }
      ii++;
...
// command: runDP_RSM_SimTimeInvariantAnalysis -designPt dp.out  -output
// results.out  -ndir $n <-experimentalPointRule Uniform -gridInfo {-1  minY
// maxY nPts 0  minY  maxY nPts0 1 minX1  maxX1 nPts1 2 minX2  maxX2 nPts2 ...}>
//  -saveHessian hession.out <-surfaceDesign UnivariateDecomposition -simulation
//  ImportanceSampling -tarCOV 0.1 -numSimulation 100000>
    ...
    ClientData clientData, Tcl_Interp *interp, int argc, TCL_Char ** const argv)
{
  ...
  if ((theRandomNumberGenerator == 0) || (theFunctionEvaluator == 0) ||
      (theProbabilityTransformation == 0) || (theGradientEvaluator == 0)) {

    opserr << "theRandomNumberGenerator "
              "==0)||(theGFunEvaluator)||(theProbabilityTransformation==0) "
              "||(theGradientEvaluator==0)"
           << endln;
    exit(-1);
  }
  ...
  while (argc > argvCounter) {
    ...
    if ((strcmp(argv[argvCounter], "-designPt") == 0) ||
        (strcmp(argv[argvCounter], "-designPoint") == 0)) {
      char fileName[20];
      argvCounter++;
      strcpy(fileName, argv[argvCounter]);
    ...
    //====  <-surfaceDesign UnivariateDecomposition -simulation
    //ImportanceSampling -tarCOV 0.1 -numSimulation 100000>
...
// command: runDP_RSM_SimTimeVariantAnalysis -designPt dp.out  -output
// results.out  -ndir $n <-experimentalPointRule Uniform -gridInfo {-1  minY
// maxY nPts 0  minY  maxY nPts0 1 minX1  maxX1 nPts1 ..}>
//  -saveHessian hession.out <-surfaceDesign UnivariateDecomposition -simulation
//  ImportanceSampling -tarCOV 0.1 -numSimulation 100000 -littleDt dt
//  -ImpulseInterval Dt>
    ...
    ClientData clientData, Tcl_Interp *interp, int argc, TCL_Char ** const argv)
{
  ...
  if ((littleDt == 0) || (ImpulseInterval == 0)) {
    ...
    opserr
        << "runDP_RSM_SimTimeVariantAnalysis -designPt dp.out  -output "
           "results.out  -ndir $n <-experimentalPointRule Uniform -gridInfo "
           "{-1  minY  maxY nPts 0  minY  maxY nPts0 1 minX1  maxX1 nPts1 ..}>"
        << endln;
    opserr << "-saveHessian hession.out <-surfaceDesign "
              "UnivariateDecomposition -simulation ImportanceSampling -tarCOV "
              "0.1 -numSimulation 100000> -littleDt dt -ImpulseInterval Dt"
           << endln;
    exit(-1);
  }

  DP_RSM_Sim_TimeVariant *theDP_RSM_Sim_TimeVariant =
...
{
  if (argc < 2) {
    opserr << "ERROR: Invalid number of arguments to getBetaFORM command."
           << endln;
    return TCL_ERROR;
  }
...
{
  if (argc < 3) {
    opserr << "ERROR: Invalid number of arguments to getGammaFORM command."
           << endln;
    return TCL_ERROR;
  }
  ...
  double gamma =
      theFunctionEvaluator->getResponseVariable("gammaFORM", lsfTag, index);
...
{
  if (argc < 3) {
    opserr << "ERROR: Invalid number of arguments to getAlphaFORM command."
           << endln;
    return TCL_ERROR;
  }
  ...
  double alpha =
      theFunctionEvaluator->getResponseVariable("alphaFORM", lsfTag, index);
                                  ...
                                  int argc, TCL_Char ** const argv)
{
  ...
  RandomVariable *theRV = theReliabilityDomain->getRandomVariablePtr(rvTag);
  if (theRV == 0) {
    opserr << "WARNING getPDF tag? x? -- random variable with tag " << rvTag
                                           ...
                                           TCL_Char ** const argv)
{
  if (argc < 2) {
    opserr
        << "WARNING getStdNormalPDF x? -- insufficient number of arguments\n";
    return TCL_ERROR;
  }
                                  ...
                                  int argc, TCL_Char ** const argv)
{
  ...
  RandomVariable *theRV = theReliabilityDomain->getRandomVariablePtr(rvTag);
  if (theRV == 0) {
    opserr << "WARNING getCDF tag? x? -- random variable with tag " << rvTag
           << " does not exist in model\n";
    return TCL_ERROR;
  }
                                           ...
                                           TCL_Char ** const argv)
{
  if (argc < 2) {
    opserr
        << "WARNING getStdNormalCDF x? -- insufficient number of arguments\n";
    return TCL_ERROR;
  }
...
{
  if (argc < 3) {
    opserr << "WARNING getInverseCDF tag? p? -- insufficient number of "
              "arguments\n";
    return TCL_ERROR;
  }
...
{
  if (argc < 2) {
    opserr << "WARNING getStdNormalInverseCDF p? -- insufficient number of "
              "arguments\n";
    return TCL_ERROR;
  }
                                      ...
                                      int argc, TCL_Char ** const argv)
{
  ...
  LimitStateFunctionIter &eleIter =
...
{
  if (argc < 2) {
    opserr << "WARNING getCutsetComponents tag? -- insufficient number of "
              "arguments\n";
    return TCL_ERROR;
  }
  ...
  if (theCutset == 0) {
    opserr << "WARNING getCutsetComponents tag? -- cutset with tag " << tag
           << " does not exist in model\n";
    return TCL_ERROR;
  }
...
/***
int
TclReliabilityModelBuilder_addAnalyzer(ClientData clientData, Tcl_Interp
*interp, int argc, TCL_Char ** const argv)
{
        int argvCounter;
        int nstep;
...
in tclModelbuidler \n"; return TCL_ERROR;
                        ...
                        opserr << "ERROR: Invalid argument to FreeVibration " <<
endln; return TCL_ERROR;
                }
        }
        if(initialstat){
                if(theInitialStaticAnalysis==0){
                        ...
                        opserr << "before defining Analyzer with
-initialstat\n"; return TCL_ERROR;
                }
        }
        if (theReliabilityDomain == 0 ) {
                opserr << "Need ReliabilityDomain before an Analyzer can be
created" << endln; return TCL_ERROR;
        }
        if (theStructuralDomain == 0 ) {
                opserr << "Need StructuralDomain before an Analyzer can be
created" << endln; return TCL_ERROR;
        }
        if (theReliabilityTransientAnalysis == 0 &&
theReliabilityStaticAnalysis==0  ) { opserr << "Need Analysis before an Analyzer
can be created" << endln; return TCL_ERROR;
        }
//	if (theSensitivityAlgorithm== 0 ) {
//		opserr << "Need SensitivityAlgorithm before an Analyzer can be
created" << endln;
//		return TCL_ERROR;
//	}
//	if (theSensitivityIntegrator== 0 ) {
//		opserr << "Need SensitivityIntegrator before an Analyzer can be
created" << endln;
//		return TCL_ERROR;
//	}
        if(defined){
...
//
argv[argvCounter],
// print); } else { opserr << "ERROR: Invalid argument to Initial Static
Analysis. " << endln; return TCL_ERROR;
        }
...
theOutCrossingAnalysis." << endln; return TCL_ERROR;
        ...
        if(analysis!=0&&analysis!=1&&analysis!=2){
                opserr << "ERROR: analysisType must be either of 0, 1, or 2" <<
endln; return TCL_ERROR;
        }
...
TCL_OK) { opserr << "ERROR: invalid input twoside to theOutCrossingAnalysis \n";
                        ...
                        opserr << "ERROR: Invalid input to
theFirstPassageAnalyzer" << endln; return TCL_ERROR;
                }
        }
        if(stationary==0){
                        opserr << "ERROR: Need to specify
stationary/nonstationary" << endln; opserr << "for FirstPassageAnalyzer" <<
endln; return TCL_ERROR;
        }

        if (theReliabilityDomain == 0) {
                opserr << "Need theReliabilityDomain before an
FirstPassageAnalyzer can be created" << endln; return TCL_ERROR;
        }
        if (theFindDesignPointAlgorithm == 0 ) {
                opserr << "Need theGFunEvaluator before an FirstPassageAnalyzer
can be created" << endln; return TCL_ERROR;
        }
        if (theFunctionEvaluator == 0 ) {
                opserr << "Need theGFunEvaluator before an FirstPassageAnalyzer
...
RandomVibrationAnalysis can be created" << endln; return TCL_ERROR;
        ...
        if (theStructuralDomain == 0 ) {
                opserr << "Need StructuralDomain before an
RandomVibrationAnalysis can be created" << endln; return TCL_ERROR;
        }
        if (theFunctionEvaluator == 0 ) {
                opserr << "Need theGFunEvaluator before an OutCrossingAnalysis
can be created" << endln; return TCL_ERROR;
        }
        if (theProbabilityTransformation == 0 ) {
                opserr << "Need theProbabilityTransformation before a
Outcrossing can be created" << endln;
                ///////////// Modified by K Fujimura
///////////////////////////////////////////// opserr << "Assume all RV's are
independent" << endln; theProbabilityTransformation = new
AllIndependentTransformation(theReliabilityDomain,0);
                ///////////// Modified by K Fujimura
/////////////////////////////////////////////
        }
...
theOutCrossingAnalysis." << endln; return TCL_ERROR;
        ...
        if(instantaneous==0&&firstpassage==0){
                opserr << "ERROR: Need to specify either instantaneous or
firstpassage" << endln; opserr << "for RandomVibrationSimulation" << endln;
                return TCL_ERROR;
        }
...
//							 theStartPoint);
                ...
                theRandomVibrationSimulation=new
NonStatRandomVibrationSimulation (theReliabilityDomain, theStructuralDomain,
                                                         theFunctionEvaluator,
                                                             theProbabilityTransformation,
                                                         StartTime,EndTime,TimeInterval,
                                                         FragMin,FragInt,nFrag,
...
//							   and FO system first
passage bool stationary = true;  // true - stationary problem bool print=false;
...
OutCrossingAnalysis can be created" << endln; return TCL_ERROR;
                }
                if (theGradientEvaluator == 0 ) {
...
//		opserr << "Need MirrorImageBuilder before a RANDOMVIBRATION can
be created" << endln;
//		return TCL_ERROR;
//		}
        }
        if(initialpoint){
                if (theInitialPointBuilder== 0 ) {
                opserr << "InitialPointBuilder is not specified before
...
theFunctionEvaluator);
        ...
        if(abs(designPoint)>1){
                if (theCrossingRateAnalyzer== 0 ) {
                opserr << "Need CrossingRateAnalyzer before a RANDOMVIBRATION
can be created" << endln; return TCL_ERROR;
                }
        }
        if(firstpassage){
                if (theFirstPassageAnalyzer== 0 ) {
                opserr << "Need FirstPassageAnalyzer before a RANDOMVIBRATION
can be created" << endln; return TCL_ERROR;
                }
        }
        ...
        theRandomVibrationAnalysis = new RandomVibrationAnalysis
                                                                (theReliabilityDomain,
                                                                 theFindDesignPointAlgorithm,
                                                                 theStructuralDomain,
// theMirrorImageBuilder, theInitialPointBuilder, theCrossingRateAnalyzer,
