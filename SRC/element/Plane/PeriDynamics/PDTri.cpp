...
PDTri::PDTri(int tag, int nd1, int nd2, int nd3,
...
PDTri::setDomain(Domain *theDomain)
{
    ...
    if (dofNd1 != 2 && dofNd1 != 3) {
      opserr << "WARNING PDTri::setDomain() element " << this->getTag() << " does not have 2 or 3 DOF at node " << theNodes[0]->getTag() << "\n";
      return;
    }
    if (dofNd2 != 2 && dofNd2 != 3) {
      opserr << "WARNING PDTri::setDomain() element " << this->getTag() << " does not have 2 or 3 DOF at node " << theNodes[1]->getTag() << "\n";
      return;
    }
    if (dofNd3 != 2 && dofNd3 != 3) {
      opserr << "WARNING PDTri::setDomain() element " << this->getTag() << " does not have 2 or 3 DOF at node " << theNodes[2]->getTag() << "\n";
      return;
    }
...
PDTri::sendSelf(int commitTag, Channel &theChannel)                                                           
{
    ...
    for (i = 0; i < numgp; i++) {
        ...
        if (res < 0) {
            opserr << "WARNING PDTri::sendSelf() - " << this->getTag() << " failed to send its Material\n";
            return res;
        }
    }
...
PDTri::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)                                 
{
    ...
    if (theMaterial == 0) {
        ...
        for (int i = 0; i < numgp; i++) {
            ...
            if (theMaterial[i] == 0) {
                opserr << "PDTri::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << "\n";
                return -1;
            }
...
double PDTri::shapeFunction(double xi, double eta)
{
...
void 
PDTri::setPressureLoadAtNodes(void)
{
    pressureLoad.Zero();
