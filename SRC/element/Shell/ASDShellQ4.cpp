...
struct AGQIParams {
  ...
  void compute(const ASDShellQ4LocalCoordinateSystem &LCS)
  {
    ...
    A1 = 0.5 * (X[1] * Y[3] + X[0] * Y[1] + X[3] * Y[0] - X[1] * Y[0] -
                X[3] * Y[1] - X[0] * Y[3]);
    A2 = 0.5 * (X[1] * Y[2] + X[0] * Y[1] + X[2] * Y[0] - X[1] * Y[0] -
                X[2] * Y[1] - X[0] * Y[2]);
    A3 = 0.5 * (X[2] * Y[3] + X[1] * Y[2] + X[3] * Y[1] - X[2] * Y[1] -
                X[3] * Y[2] - X[1] * Y[3]);
    A  = A1 + A3;
...
public:
  ...
  Vector RHS_winertia =
      Vector(24); // RHS vector (residual vector with inertia terms)
...
void computeBdrilling(const ASDShellQ4LocalCoordinateSystem &LCS, double xi,
                      double eta, const JacobianOperator &Jac,
                      const AGQIParams &agq, const Vector &N, const Matrix &dN,
                      Vector &Bd)
{
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (int ii = 0; ii < 4; ii++) {
      ...
      if (mm > 3)
        mm = 0;
      SX = SX + agq.b[ii] * NXI[ii] * NETA[ii] *
                    (3.0 * (L[jj] - L[mm]) + (agq.g[jj] - agq.g[kk]));
      SY = SY + agq.c[ii] * NXI[ii] * NETA[ii] *
                    (3.0 * (L[jj] - L[mm]) + (agq.g[jj] - agq.g[kk]));
    }
    dNdX(i, 0) = (agq.b[i] + agq.b[j]) / agq.A / 2.0 +
                 NXI[i] * NETA[i] * agq.g[k] * SX / 2.0 / agq.A /
                     (1.0 + agq.g[0] * agq.g[2] + agq.g[1] * agq.g[3]);
    dNdX(i, 1) = (agq.c[i] + agq.c[j]) / agq.A / 2.0 +
                 NXI[i] * NETA[i] * agq.g[k] * SY / 2.0 / agq.A /
                     (1.0 + agq.g[0] * agq.g[2] + agq.g[1] * agq.g[3]);
  }
...
void computeBMatrix(const ASDShellQ4LocalCoordinateSystem &LCS, double xi,
                    double eta, const JacobianOperator &Jac,
                    const AGQIParams &agq, const MITC4Params &mitc,
                    const Vector &N, const Matrix &dN, const Matrix &BQ_mean,
                    Matrix &B, Matrix &BQ, Vector &Bd)
{
  // cartesian derivatives of standard shape function
  auto &dNdX = ASDShellQ4Globals::instance().dNdX;
  dNdX.addMatrixProduct(0.0, dN, Jac.invJ, 1.0);
...
ASDShellQ4::ASDShellQ4(int tag, int node1, int node2, int node3, int node4,
                       SectionForceDeformation *section, bool corotational)
    : Element(tag, ELE_TAG_ASDShellQ4),
      m_transformation(corotational ? new ASDShellQ4CorotationalTransformation()
                                    : new ASDShellQ4Transformation())
{
  // save node ids
  m_node_ids(0) = node1;
  m_node_ids(1) = node2;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (m_sections[i] == 0) {
      opserr << "ASDShellQ4::constructor - failed to get a material of type: "
                "ShellSection\n";
      exit(-1);
    }
  }
...
int ASDShellQ4::getNumExternalNodes() const { return 4; }

const ID &ASDShellQ4::getExternalNodes() { return m_node_ids; }

Node **ASDShellQ4::getNodePtrs(void)
{
  return m_transformation->getNodes().data();
}

int ASDShellQ4::getNumDOF() { return 24; }
...
int ASDShellQ4::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  opserr << "ASDShellQ4::addLoad - load type unknown for ele with tag: "
         << this->getTag() << endln;
  return -1;
}

int ASDShellQ4::addInertiaLoadToUnbalance(const Vector &accel)
{
...
int ASDShellQ4::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (res < 0) {
    opserr << "WARNING ASDShellQ4::sendSelf() - " << this->getTag()
           << " failed to send ID\n";
    return res;
  }
...
int ASDShellQ4::recvSelf(int commitTag, Channel &theChannel,
                         FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING ASDShellQ4::recvSelf() - " << this->getTag()
  ...
  if (m_transformation)
    delete m_transformation;
  m_transformation = idData(13) == 0
                         ? new ASDShellQ4Transformation()
                         : new ASDShellQ4CorotationalTransformation();
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (m_sections[i] == 0) {
      opserr << "ASDShellQ4::recvSelf() - Broker could not create NDMaterial "
                "of class type"
             << matClassTag << endln;
      ;
      return -1;
    ...
    if (res < 0) {
      opserr << "ASDShellQ4::recvSelf() - material " << i
...
Response *ASDShellQ4::setResponse(const char **argv, int argc,
                                  OPS_Stream &output)
{
  ...
  if (strcmp(argv[0], "force") == 0 || strcmp(argv[0], "forces") == 0 ||
    ...
    if (pointNum > 0 && pointNum <= 4) {
      ...
      theResponse =
          m_sections[pointNum - 1]->setResponse(&argv[2], argc - 2, output);
...
int ASDShellQ4::getResponse(int responseID, Information &eleInfo)
{
...
int ASDShellQ4::calculateAll(Matrix &LHS, Vector &RHS, int options)
{
  ...
  m_transformation->transformToGlobal(local_cs, UG, UL, LHS, RHS,
                                      (options & OPT_LHS));
...
void ASDShellQ4::AGQIbeginGaussLoop(
    const ASDShellQ4LocalCoordinateSystem &reference_cs)
{
  ...
  for (int igauss = 0; igauss < 4; igauss++) {
    ...
    // area coordinates of the gauss point (Eq 7)
    L[0] =
        0.25 * (1.0 - xi) * (agq.g[1] * (1.0 - eta) + agq.g[2] * (1.0 + eta));
    L[1] = 0.25 * (1.0 - eta) * (agq.g[3] * (1.0 - xi) + agq.g[2] * (1.0 + xi));
    L[2] =
        0.25 * (1.0 + xi) * (agq.g[0] * (1.0 - eta) + agq.g[3] * (1.0 + eta));
    L[3] = 0.25 * (1.0 + eta) * (agq.g[0] * (1.0 - xi) + agq.g[1] * (1.0 + xi));
...
int ASDShellQ4::displaySelf(Renderer &theViewer, int displayMode, float fact,
