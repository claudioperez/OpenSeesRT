...
void *
OPS_DECL_RUNTIME_VPID(OPS_DispBeamColumnNL3d, const ID &info)
{
  // data
  int iData[5];
  int numData;
...
DispBeamColumnNL3d::update(void)
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    double thetay =
        (3 * zeta * zeta - 4 * zeta + 1) * v(3) + (3 * zeta * zeta - 2 * zeta) * v(4);

    int j;
    for (j = 0; j < order; j++) {
      switch (code(j)) {
      case SECTION_RESPONSE_P:
        e(j) = oneOverL * v(0) + 0.5 * thetaz * thetaz + 0.5 * thetay * thetay;
        break;
...
DispBeamColumnNL3d::commitSensitivity(int gradNumber, int numGrads)
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    for (int j = 0; j < order; j++) {
      switch (code(j)) {
      case SECTION_RESPONSE_P:
        e(j) = oneOverL * dvdh(0) + d1oLdh * v(0) + theta * (c1 * dvdh(1) + c2 * dvdh(2));
        break;
      case SECTION_RESPONSE_MZ:
