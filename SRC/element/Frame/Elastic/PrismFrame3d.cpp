...
Matrix PrismFrame3d::K(12,12);
Vector PrismFrame3d::P(12);
...
PrismFrame3d::PrismFrame3d(int tag, double a, double e, double g, 
  ...
  :Element(tag,ELE_TAG_ElasticBeam3d), 
   ...
   Q(12), wx(0.0), wy(0.0), wz(0.0),
   connectedExternalNodes(2), theCoordTransf(nullptr)
{
...
PrismFrame3d::PrismFrame3d(int tag, int Nd1, int Nd2, SectionForceDeformation *section,  
                             CrdTransf &coordTransf, double rho_, int cm, int relz, int rely)
  :Element(tag,ELE_TAG_ElasticBeam3d), 
   releasez(relz), releasey(rely),
   cMass(cm), rho(rho_),
   Q(12), wx(0.0), wy(0.0), wz(0.0),
...
int
PrismFrame3d::getNumExternalNodes() const
{
  return 2;
}

const ID &
PrismFrame3d::getExternalNodes() 
{
  return connectedExternalNodes;
}

Node **
PrismFrame3d::getNodePtrs() 
{
  return theNodes;
}

int
PrismFrame3d::getNumDOF()
{
  return 12;
}

void
PrismFrame3d::setDomain(Domain *theDomain)
{
    if (theDomain == nullptr) {
      opserr << "PrismFrame3d::setDomain -- Domain is null\n";
      return;
    }
    ...
    if (theNodes[0] == nullptr) {
      opserr << "PrismFrame3d::setDomain  tag: " << this->getTag() << " -- Node 1: " << connectedExternalNodes(0) << " does not exist\n";
      exit(-1);
    }
                              
    if (theNodes[1] == nullptr) {
      opserr << "PrismFrame3d::setDomain  tag: " << this->getTag() << " -- Node 2: " << connectedExternalNodes(1) << " does not exist\n";
      exit(-1);
    }
    ...
    if (dofNd1 != 6) {
      opserr << "PrismFrame3d::setDomain  tag: " << this->getTag() << " -- Node 1: " << connectedExternalNodes(0) 
             << " has incorrect number of DOF\n";
      exit(-1);
    }
    
    if (dofNd2 != 6) {
      opserr << "PrismFrame3d::setDomain  tag: " << this->getTag() << " -- Node 2: " << connectedExternalNodes(1) 
             << " has incorrect number of DOF\n";
      exit(-1);
    }
    ...
    if (theCoordTransf->initialize(theNodes[0], theNodes[1]) != 0) {
        opserr << "PrismFrame3d::setDomain  tag: " << this->getTag() << " -- Error initializing coordinate transformation\n";
        exit(-1);
    }
...
//q.addMatrixVector(0.0, kb, v, 1.0);
  q  = kb*v;
  q += q0;
  // TODO
  Matrix ktemp(kb);
...
int 
PrismFrame3d::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  int type;
  ...
  if (type == LOAD_TAG_Beam3dUniformLoad) {
    ...
    opserr << "PrismFrame3d::addLoad()  -- load type unknown for element with tag: " << this->getTag() << "\n";
    return -1;
  }
...
int
PrismFrame3d::addInertiaLoadToUnbalance(const Vector &accel)
{
  if (rho == 0.0)
    return 0;
  ...
  if (6 != Raccel1.Size() || 6 != Raccel2.Size()) {
    opserr << "PrismFrame3d::addInertiaLoadToUnbalance matrix and vector sizes are incompatible\n";
    return -1;
  }
...
const Vector &
PrismFrame3d::getResistingForceIncInertia()
{        
...
PrismFrame3d::recvSelf(int cTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  ...
  if (theCoordTransf == 0) {
    ...
    if (theCoordTransf == 0) {
      opserr << "PrismFrame3d::recvSelf -- could not get a CrdTransf3d\n";
      exit(-1);
    }
  }
