...
// Revision: B
//
// Purpose: This file contains the class definition for PrismShearFrame2d.
// PrismShearFrame2d is a 2d beam element. As such it can only
...
void * OPS_ADD_RUNTIME_VPV(OPS_PrismShearFrame2d)
{
    ...
    if (numRemainingArgs == 0)  { // parallel processing
        theElement = new PrismShearFrame2d();
        return theElement;
    }
    
    if (numRemainingArgs < 9)  {
        opserr << "ERROR not enough args provided, want: element PrismShearFrame2d $tag $iNode $jNode $E $G $A $Iz $Avy $transTag <-mass $m> <-cMass> \n";
        return 0;
    }
    ...
    if (OPS_GetIntInput(&numData, iData) != 0)  {
        opserr << "WARNING invalid element data (tag, iNode, jNode) element PrismShearFrame2d.\n";
        return 0;
    }
    
    numData = 5;
    if (OPS_GetDoubleInput(&numData, dData) != 0) {
        opserr << "WARNING error reading element data (E, G, A, Iz, Avy) element PrismShearFrame2d " << iData[0] << endln;
        return 0;
    }
    
    numData = 1;
    if (OPS_GetIntInput(&numData, &iData[3]) != 0)  {
        opserr << "WARNING invalid element data (transTag) element PrismShearFrame2d " << iData[0] << endln;
        return 0;
    }
    
    CrdTransf *theTrans = G3_getSafeBuilder(rt)->getTypedObject<CrdTransf>(iData[3]);
    if (theTrans == 0)  {
        opserr << "WARNING transformation object not found for PrismShearFrame2d " << iData[0] << endln;
        return 0;
    }
      ...
      const char *argvLoc = OPS_GetString();
        ...
        if ((strcmp(argvLoc, "-mass") == 0) || (strcmp(argvLoc, "mass") == 0) ||
            ...
            if (OPS_GetDoubleInput(&numData, &dData[5]) != 0)  {
                opserr << "WARNING error reading element data (mass) element PrismShearFrame2d " << iData[0] << endln;
                return 0;
            }
        }
    ...
    theElement = new PrismShearFrame2d(iData[0], iData[1], iData[2],
...
PrismShearFrame2d::PrismShearFrame2d(int tag, int Nd1, int Nd2, 
    ...
    Avy(avy), rho(r), cMass(cm), geom_flag(0), phi(0.0), L(0.0), ul(6), ql(6),
    kl(6,6), klgeo(6,6), Tgl(6,6), Ki(6,6), M(6,6), theLoad(6)
{
    // ensure the connectedExternalNode ID is of correct size & set values
    if (connectedExternalNodes.Size() != 2)  {
        opserr << "PrismShearFrame2d::PrismShearFrame2d() - element: "
...
PrismShearFrame2d::PrismShearFrame2d()
    ...
    Iz(0.0), Avy(0.0), rho(0.0), cMass(0), geom_flag(0), phi(0.0), L(0.0), ul(6),
    kl(6,6), klgeo(6,6), Tgl(6,6), Ki(6,6), M(6,6), theLoad(6)
{
    // ensure the connectedExternalNode ID is of correct size & set values
    if (connectedExternalNodes.Size() != 2)  {
...
int PrismShearFrame2d::getNumExternalNodes() const
{
    return 2;
}


const ID& PrismShearFrame2d::getExternalNodes()
{
    return connectedExternalNodes;
}


Node** PrismShearFrame2d::getNodePtrs()
{
    return theNodes;
}


int PrismShearFrame2d::getNumDOF()
{
    return 6;
}


void PrismShearFrame2d::setDomain(Domain *theDomain)
{
    ...
    if (!theNodes[0] || !theNodes[1])  {
        if (!theNodes[0])  {
            opserr << "PrismShearFrame2d::setDomain() - Nd1: "
                << connectedExternalNodes(0)
                << " does not exist in the model for";
        } else  {
            opserr << "PrismShearFrame2d::setDomain() - Nd2: "
    ...
    if (dofNd1 != 3)  {
        opserr << "PrismShearFrame2d::setDomain() - node 1: "
            << connectedExternalNodes(0)
            << " has incorrect number of DOF (not 3).\n";
        return;
    }
    if (dofNd2 != 3)  {
        opserr << "PrismShearFrame2d::setDomain() - node 2: "
            << connectedExternalNodes(1)
            << " has incorrect number of DOF (not 3).\n";
        return;
    }
...
int PrismShearFrame2d::commitState()
{
    int errCode = 0;
...
int PrismShearFrame2d::revertToLastCommit()
{
    return 0;
}


int PrismShearFrame2d::revertToStart()
{
    return 0;
}


int PrismShearFrame2d::update()
{
    return 0;
}


const Matrix& PrismShearFrame2d::getTangentStiff()
{
    // zero the matrix
    theMatrix.Zero();
...
const Matrix& PrismShearFrame2d::getInitialStiff()
{
    return Ki;
}


const Matrix& PrismShearFrame2d::getMass()
{ 
    return M;
}


void PrismShearFrame2d::zeroLoad()
{
    theLoad.Zero();
    ql0.Zero();
...
int PrismShearFrame2d::addLoad(ElementalLoad *load, double loadFactor)
{
    int type;
...
int PrismShearFrame2d::addInertiaLoadToUnbalance(const Vector &accel)
{
...
const Vector& PrismShearFrame2d::getResistingForce()
{
    // zero the residual
    theVector.Zero();
...
const Vector& PrismShearFrame2d::getResistingForceIncInertia()
{
...
int PrismShearFrame2d::sendSelf(int commitTag, Channel &sChannel)
{
    int res = 0;
...
int PrismShearFrame2d::recvSelf(int commitTag, Channel &rChannel,
...
void PrismShearFrame2d::Print(OPS_Stream &s, int flag)
{
...
Response* PrismShearFrame2d::setResponse(const char **argv, int argc,
    OPS_Stream &output)
{
...
int PrismShearFrame2d::getResponse (int responseID, Information &eleInfo)
{
    switch (responseID) {
...
int PrismShearFrame2d::setParameter(const char **argv,
...
int PrismShearFrame2d::updateParameter (int parameterID,
    Information &info)
{
    switch (parameterID) {
    case -1:
