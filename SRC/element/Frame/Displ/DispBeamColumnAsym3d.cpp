...
DispBeamColumnAsym3d::DispBeamColumnAsym3d(int tag, int nd1, int nd2, int numSec,
                                           ...
                                           int cm) //Xinlong
    : Element(tag, ELE_TAG_DispBeamColumnAsym3d), numSections(numSec), theSections(0),
      crdTransf(0), beamInt(0), connectedExternalNodes(2), Q(12), q(6), ys(yss), zs(zss),
      rho(r), cMass(cm), parameterID(0) //Xinlong
{
  ...
  if (theSections == 0) {
    opserr << "DispBeamColumnAsym3d::DispBeamColumnAsym3d - failed to allocate section "
              "model pointer\n";
    exit(-1);
  }
  ...
  if (beamInt == 0) {
    opserr << "DispBeamColumnAsym3d::DispBeamColumnAsym3d - failed to copy beam "
              "integration\n";
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "DispBeamColumnAsym3d::DispBeamColumnAsym3d - failed to copy coordinate "
              "transformation\n";
    exit(-1);
  }
...
DispBeamColumnAsym3d::update(void)
{
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    e(0) = s0 +
           (4.0 * s7 * s7 + 4.0 * s8 * s8 + 4.0 * s9 * s9 + 4.0 * s10 * s10 -
            2.0 * s7 * s9 - 2.0 * s8 * s10) /
               60.0 +
           (zs * s4 - ys * s5) * s3;
    e(1) = s1 + s2 * s6;
    e(2) = -s2 + s1 * s6;
