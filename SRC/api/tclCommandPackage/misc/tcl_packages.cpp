  ...
  *funcHandle = NULL;
  ...
  if (hLib != NULL) {
    ...
    if (*funcHandle == NULL) {
      ...
      (*funcHandle) =
          (void *)GetProcAddress((HMODULE)hLib, underscoreFunctionName);
    ...
    // typedef int(_cdecl* OPS_ResetInputType)(ClientData, Tcl_Interp*, int,
    // int, TCL_Char**, Domain*, TclBasicBuilder*); typedef int(_cdecl*
    // OPS_ResetInputNoBuilderType)(ClientData, Tcl_Interp*, int, int,
    // TCL_Char**, Domain*);
    ...
    typedef int(_cdecl * OPS_InvokeMaterialDirectlyPtrType)(
        matObject **, modelState *, double *, double *, double *, int *);
    ...
    typedef void(_cdecl * setGlobalPointersFunction)(
        ...
        OPS_GetNodeInfoPtrType, OPS_GetNodeInfoPtrType, OPS_GetNodeInfoPtrType,
        OPS_GetNumRemainingInputArgsType, OPS_ResetCurrentInputArgType,
        // OPS_ResetInputType,
        // OPS_ResetInputNoBuilderType,
        OPS_GetStringType, OPS_GetStringCopyType, OPS_GetIntPtrType,
        OPS_GetIntPtrType, OPS_GetFEDatastorePtrType, OPS_GetInterpPWD_PtrType,
        OPS_GetAnalysisModelPtrType, OPS_GetAlgorithmPtrType,
        OPS_GetHandlerPtrType, OPS_GetNumbererPtrType, OPS_GetSOEPtrType,
        OPS_GetEigenSOEPtrType, OPS_GetStaticAnalysisPtrType,
        OPS_GetTransientAnalysisPtrType,
        OPS_GetVariableTimeStepTransientAnalysisPtrType, OPS_GetNumEigenPtrType,
        OPS_GetStaticIntegratorPtrType, OPS_GetTransientIntegratorPtrType,
        OPS_GetTestPtrType, OPS_builtModelPtrType, OPS_GetDomainPointerType);

    setGlobalPointersFunction funcPtr;

    // look for pointer function
    funcPtr = (setGlobalPointersFunction)GetProcAddress((HMODULE)hLib,
                                                        "setGlobalPointers");
    if (funcPtr == 0) {
      FreeLibrary((HMODULE)hLib);
      return -2;
    ...
    (funcPtr)(opserrPtr, ops_TheActiveDomain, theSimulationInfoPtr, OPS_Error,
              ...
              OPS_GetNumRemainingInputArgs, OPS_ResetCurrentInputArg,
              // OPS_ResetInput,
              // OPS_ResetInputNoBuilder,
              OPS_GetString, OPS_GetStringCopy, OPS_GetNDM, OPS_GetNDF,
              OPS_GetFEDatastore, OPS_GetInterpPWD, OPS_GetAnalysisModel,
              OPS_GetAlgorithm, OPS_GetHandler, OPS_GetNumberer, OPS_GetSOE,
              OPS_GetEigenSOE, OPS_GetStaticAnalysis, OPS_GetTransientAnalysis,
