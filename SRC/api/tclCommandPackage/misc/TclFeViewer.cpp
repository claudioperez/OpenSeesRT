...
//
// the functions that will be invoked by the interpreter while building the
// model
//
...
int TclFeViewer_setVUP(ClientData clientData, Tcl_Interp *interp, int argc,
                       TCL_Char **argv);
int TclFeViewer_setViewWindow(ClientData clientData, Tcl_Interp *interp,
                              int argc, TCL_Char **argv);
int TclFeViewer_setPlaneDist(ClientData clientData, Tcl_Interp *interp,
                             int argc, TCL_Char **argv);
...
int TclFeViewer_setPRP(ClientData clientData, Tcl_Interp *interp, int argc,
                       TCL_Char **argv);
int TclFeViewer_setPortWindow(ClientData clientData, Tcl_Interp *interp,
                              int argc, TCL_Char **argv);
int TclFeViewer_displayModel(ClientData clientData, Tcl_Interp *interp,
                             int argc, TCL_Char **argv);
...
TclFeViewer::TclFeViewer()
    : Recorder(RECORDER_TAGS_TclFeViewer), theMap(0), theRenderer(0),
      theDomain(0), theEleMode(-1), theNodeMode(-1), theDisplayFact(1),
      deltaT(0.0), nextTimeStampToRecord(0.0), wipeFlag(0), vrpSet(0),
      vpwindowSet(0), clippingPlaneDistancesSet(0)
{
  theTclFeViewer = 0;
  theMap = 0;
}
...
TclFeViewer::TclFeViewer(const char *title, int xLoc, int yLoc, int width,
    ...
    : Recorder(RECORDER_TAGS_TclFeViewer), theMap(0), theRenderer(0),
      ...
      wipeFlag(WipeFlag), vrpSet(0), vpwindowSet(0),
      clippingPlaneDistancesSet(0)
{
...
#ifdef _WGL
  theRenderer = new OpenGLRenderer(title, xLoc, yLoc, width, height, *theMap, 0,
                                   fileName);
#elif _GLX
  theRenderer = new OpenGLRenderer(title, xLoc, yLoc, width, height, *theMap,
                                   fileName, 0);
...
#else
  theRenderer =
      new X11Renderer(title, xLoc, yLoc, width, height, *theMap, fileName);
#endif
...
TclFeViewer::record(int cTag, double timeStamp)
{
  ...
  if (deltaT == 0.0 || timeStamp >= nextTimeStampToRecord) {
    ...
    if (vrpSet == 0 || vpwindowSet == 0 || clippingPlaneDistancesSet == 0) {
      ...
      if (vpwindowSet == 0)
        this->setViewWindow(float(-diff), float(diff), float(-diff),
                            float(diff));
    ...
    if (theEleMode != 0) {
      ...
      while ((theEle = theElements()) != 0) {
        res = theEle->displaySelf(*theRenderer, theEleMode,
                                  float(theDisplayFact));
        if (res < 0) {
    ...
    if (theNodeMode != 0) {
      ...
      while ((theNode = theNodes()) != 0) {
        res = theNode->displaySelf(*theRenderer, theEleMode, theNodeMode,
                                   float(theDisplayFact));
        if (res < 0) {
                          ...
                          TCL_Char **argv)
{
  ...
  theTclFeViewer->setViewWindow(float(uMin), float(uMax), float(vMin),
                                float(vMax));
  return TCL_OK;
#endif
}
...
int
TclFeViewer_setProjectionMode(ClientData clientData, Tcl_Interp *interp,
                              int argc, TCL_Char **argv)
{
#ifdef _NOGRAPHICS
  // if no graphics .. just return 0
                          ...
                          TCL_Char **argv)
{
  ...
  theTclFeViewer->setPortWindow(float(uMin), float(uMax), float(vMin),
                                float(vMax));
  return TCL_OK;
#endif
}
                         ...
                         TCL_Char **argv)
{
  ...
  if (argc < 3 || argc > 5) {
    opserr << "WARNING args incorrect - display eleMode <nodeMode> displayFact "
              "<lineWidth>\n";
    return TCL_ERROR;
  }
                      ...
                      TCL_Char **argv)
{
  ...
  // check number of args
  if (argc != 3 && argc != 5) {
    opserr << "WARNING args incorrect - saveImage -image imageName? -file "
              "fileName?\n";
    return TCL_ERROR;
  }
  ...
  while (loc < argc) {
    if (strcmp(argv[loc], "-image") == 0)
      ...
      opserr << "WARNING invalid option: " << argv[loc]
             << " - saveImage -image imageName? -file fileName?\n";
      return TCL_ERROR;
    }
    loc += 2;
  }
...
int
TclFeViewer::recvSelf(int commitTag, Channel &theChannel,
