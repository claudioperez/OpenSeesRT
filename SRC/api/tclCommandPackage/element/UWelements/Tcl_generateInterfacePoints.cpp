                                   ...
                                   int argc, TCL_Char **argv)
{
  ...
  int lagrangeTag =
      8; // 0 = local penalty, 1 = local Augmented Lagrangian, 2 = global
         // Lagrange, 3 = global embedded lagrange, 4 = global penalty
  ...
  while (numArgsRemaining > 0) {
    if ((strcmp(argv[curArgPos], "-shape") == 0) && numArgsRemaining >= 2) {
      ...
      if (strcmp(argv[curArgPos], "circle") == 0) {
        ...
        numArgsRemaining--;
        curArgPos++;
        opserr << "The procedure for square shapes is not implemented yet."
               << endln;
        ...
        numArgsRemaining--;
        curArgPos++;
        opserr << "The procedure for rectangle shapes is not implemented yet."
               << endln;
        ...
        numArgsRemaining--;
        curArgPos++;
        opserr << "The procedure for hexagon shapes is not implemented yet."
               << endln;
      } else {
        numArgsRemaining--;
        curArgPos++;
    ...
    } else if ((strcmp(argv[curArgPos], "-radius") == 0) &&
               numArgsRemaining >= 2) {
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
      readingBeamEleTag = false;
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-crdTransf") == 0) &&
               numArgsRemaining >= 2) {
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
      readingBeamEleTag = false;
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-penaltyParam") == 0) &&
               numArgsRemaining >= 2) {
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
      readingBeamEleTag = false;
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-solidEle") == 0) &&
               numArgsRemaining >= 2) {
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-solidEleRange") == 0) &&
               numArgsRemaining >= 3) {
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
      readingBeamEleTag = false;
    ...
    } else if ((strcmp(argv[curArgPos], "-beamEle") == 0) &&
               numArgsRemaining >= 2) {
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-beamEleRange") == 0) &&
               numArgsRemaining >= 3) {
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
      readingBeamEleTag = false;
    ...
    } else if ((strcmp(argv[curArgPos], "-connectedBeams") == 0) &&
               numArgsRemaining >= 2) {
      readingSolidEleTag = false;
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-file") == 0) &&
               numArgsRemaining >= 2) {
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
      readingBeamEleTag = false;
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-connectivity") == 0) &&
               numArgsRemaining >= 2) {
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
      readingBeamEleTag = false;
      ...
      curArgPos++;
               ...
               (strcmp(argv[curArgPos], "-lPenalty") == 0)) {
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
      readingBeamEleTag = false;
      // 0 = local penalty, 1 = local Augmented Lagrangian, 2 = global Lagrange,
      // 3 = global embedded lagrange, 4 = global penalty
      lagrangeTag = 0;
      numArgsRemaining--;
      curArgPos++;
      ...
      curArgPos++;
               ...
               (strcmp(argv[curArgPos], "-gLagrange") == 0)) {
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
      readingBeamEleTag = false;
      // 0 = local penalty, 1 = local Augmented Lagrangian, 2 = global Lagrange,
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-mat") == 0) &&
               (numArgsRemaining >= 2)) {
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-quadFileInfo") == 0) &&
               (numArgsRemaining >= 3)) {
      // This one is for the elastoplastic interface
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-baseNode") == 0) &&
               (numArgsRemaining >= 2)) {
      // This one is for the elastoplastic interface
      readingSolidEleTag = false;
      readingConBeamEleTag = false;
  ...
  // Loop over all the elements and check if they are supported. If yes add them
  // to the element pool
  ...
  for (int beamCount = 0; beamCount < beamEleTags.size(); beamCount++) {
    ...
    if (!((strcmp(beamEleClassType, "ElasticBeam3d") == 0) ||
          ...
          (strcmp(beamEleClassType, "ForceBeamColumn3d") == 0))) {
      opserr << "Beam element of type " << theElement->getClassType()
             << "not supported." << endln;
      return -1;
    }
    ...
    // Use either the transformation object (next block of code) or calculate
    // rotations and everything (below)

    // // calculate the rotation information(the centerline is calculated using
    // a
    // // rotation operation)
    // double ux = L2x - L1x;
    // double uy = L2y - L1y;
    ...
    // 	}
    //
    // Vector cXr = cX*costheta*singamma - cY*sintheta + cZ*costheta*cosgamma +
    // L1x; Vector cYr = cX*sintheta*singamma + cY*costheta +
    // cZ*sintheta*cosgamma + L1y; Vector cZr = cZ*singamma - cX*cosgamma + L1z;
    ...
    if (theTransf->initialize(beamNodePtr[0], beamNodePtr[1])) {
      opserr << "generateInterfacePoints: Error initializing coordinate "
                "transformation";
      return -1;
    }
    ...
    double L =
        sqrt(pow(L2x - L1x, 2.0) + pow(L2y - L1y, 2.0) + pow(L2z - L1z, 2.0));
    ...
    if (writeQuadInfo) {
      ...
      for (int ii = 0; ii < nL + 1; ii++)
        for (int jj = 0; jj < nP + 1; jj++) {
          ...
          quadNodes << tagOffset + beamCount * (nL + 1) * (nP + 1) +
                           ii * (nP + 1) + jj
                    << " " << qcXr << " " << qcYr << " " << qcZr << endln;
        }
    }
    ...
    if (debugFlag)
      for (int ii = 0; ii < nL; ii++)
        for (int jj = 0; jj < nP; jj++)
          opserr << "point " << ii * nP + jj + 1 << " : " << cXr(ii * nP + jj)
                 << " " << cYr(ii * nP + jj) << " " << cZr(ii * nP + jj)
                 << endln;
    ...
    for (int jj = 0; jj < nP * nL; jj++) {
      for (int ii = 0; ii < solidEleTags.size(); ii++) {
        ...
        for (int kk = 0; kk < 8; kk++) {
          tempX(kk) =
              solidNodesPtr[kk]->getCrds()(0) + solidNodesPtr[kk]->getDisp()(0);
          tempY(kk) =
              solidNodesPtr[kk]->getCrds()(1) + solidNodesPtr[kk]->getDisp()(1);
          tempZ(kk) =
              solidNodesPtr[kk]->getCrds()(2) + solidNodesPtr[kk]->getDisp()(2);
        }
        invIsoMapping(tempX, tempY, tempZ, cXr(jj), cYr(jj), cZr(jj), xi, eta,
                      zeta, inBounds);
        if (inBounds) {
          contactElemFlag = true;
          if (debugFlag)
            opserr << "Beam tag : " << beamTag
                   << ", Solid tag : " << solidEleTags[ii]
                   << ", Real Coordinates = (" << cXr(jj) << "," << cYr(jj)
                   << "," << cZr(jj) << "), Iso Coordinates = (" << xi << ","
                   << eta << "," << zeta << ")" << endln;
          ...
          if (lagrangeTag > 1) {
            ...
            // 0 = local penalty, 1 = local Augmented Lagrangian, 2 = global
            // Lagrange, 3 = global embedded lagrange
            if (lagrangeTag == 0) {
              // theElement = new EmbeddedBeamInterface(maxTag, beamTag,
              // solidEleTags[ii], transfTag, loc_rho(jj), loc_theta(jj), xi,
              // eta, zeta, radius, area, writeConnectivity, connectivityFN);
            } else if (lagrangeTag == 1) {
              // theElement = new EmbeddedBeamInterfaceAL(maxTag, beamTag,
              // solidEleTags[ii], transfTag, loc_rho(jj), loc_theta(jj), xi,
              // eta, zeta, radius, area);
            }
            if (lagrangeTag == -1) {
              // theElement = new EmbeddedBeamContact(maxTag, beamTag,
              // solidEleTags[ii], transfTag, loc_rho(jj), loc_theta(jj), xi,
              // eta, zeta, radius, area);
            }
          ...
          if (writeCoords)
            crdsFile << maxTag << "\t" << cXr(jj) << "\t" << cYr(jj) << "\t"
                     << cZr(jj) << "\n";

          if (writeQuadInfo) {
            int setNum = (int)(jj / nP);
            int ptNum = (int)(jj % nP);
            int pt1 = tagOffset + beamCount * (nL + 1) * (nP + 1) +
                      setNum * (nP + 1) + ptNum;
            int pt3 = tagOffset + beamCount * (nL + 1) * (nP + 1) +
                      (setNum + 1) * (nP + 1) + ptNum;

            quadElems << contactPointCount << " " << pt1 << " " << pt1 + 1
                      << " " << pt3 + 1 << " " << pt3 << endln;
          }
  ...
  if (contactPointCount > 0) {
    ...
    // 0 = local penalty, 1 = local Augmented Lagrangian, 2 = global Lagrange, 3
    // = global embedded lagrange
    if (lagrangeTag > 1) {
      if (lagrangeTag == 2)
        theElement = new EmbeddedBeamInterfaceL(
            maxTag, beamEleTagsInContact, solidEleTagsInContact, transfTag,
            contactPt_rho, contactPt_theta, contactPt_xi, contactPt_eta,
            contactPt_zeta, radius, contactPt_area, contactPt_length,
            writeConnectivity, connectivityFN);
      else if (lagrangeTag == 3) {
        //    if (beamConnected)
        //        theElement = new EmbeddedBeamInterfaceAL2(maxTag,
        //        beamEleTagsInContact, solidEleTagsInContact, transfTag,
        //        contactPt_rho, contactPt_theta, contactPt_xi, contactPt_eta,
        //        contactPt_zeta, radius, contactPt_area, writeConnectivity,
        //        connectivityFN);
        //    else
        //        theElement = new EmbeddedBeamInterfaceAL2(maxTag,
        //        beamEleTagsInContact, solidEleTagsInContact, transfTag,
        ...
        theElement = new EmbeddedBeamInterfaceP(
            maxTag, beamEleTagsInContact, solidEleTagsInContact, transfTag,
            contactPt_rho, contactPt_theta, contactPt_xi, contactPt_eta,
            contactPt_zeta, radius, contactPt_area, contactPt_length,
            penaltyParam, writeConnectivity, connectivityFN);
      else if (lagrangeTag == 5) {
        // theElement = new EmbeddedBeamInterfaceAL2(maxTag, beamTag,
        // solidEleTagsInContact, transfTag, contactPt_rho, contactPt_theta,
        // contactPt_xi, contactPt_eta, contactPt_zeta, radius, area);
        //    theElement = new EmbeddedBeamInterfaceP2(maxTag, beamTag,
        //    solidEleTagsInContact, transfTag, contactPt_rho, contactPt_theta,
        //    contactPt_xi, contactPt_eta, contactPt_zeta, radius, area);
      } else if (lagrangeTag == 6) {
        theElement = new EmbeddedEPBeamInterface(
            maxTag, beamEleTagsInContact, solidEleTagsInContact, transfTag,
            matTag, contactPt_rho, contactPt_theta, contactPt_xi, contactPt_eta,
            contactPt_zeta, radius, contactPt_area, contactPt_length,
            writeConnectivity, connectivityFN);
      } else if (lagrangeTag == 7) {
        //    theElement = new EmbeddedBeamContactInterfaceP(maxTag, beamTag,
        //    solidEleTagsInContact, transfTag, contactPt_rho, contactPt_theta,
        //    contactPt_xi, contactPt_eta, contactPt_zeta, radius, area);
        // theElement = new EmbedBeamContactG(maxTag, beamTag,
        // solidEleTagsInContact, transfTag, contactPt_rho, contactPt_theta,
        // contactPt_xi, contactPt_eta, contactPt_zeta, radius, area);
      }
  ...
  opserr << "Number of elements in contact: "
         << (int)eleTagsInContact_unique.size()
         << ", number of contact points: " << contactPointCount << endln;
  opserr << "EmbeddedBeamInterface Elements " << startTag << " to " << maxTag
         << " were created." << endln;
                                      ...
                                      int argc, TCL_Char **argv)
{
  ...
  bool debugFlag = false;
  bool writeCoords = false;
  int lagrangeTag = 3; // 0 = local penalty, 1 = local Augmented Lagrangian, 2 =
                       // global Lagrange, 3 = global embedded lagrange
  ...
  if (numArgsRemaining < 3) {
    opserr
        << "Need beam tag, geometric transformation tag and the toe node tag."
        << endln;
    return -1;
  } else {
  ...
  while (numArgsRemaining > 0) {
    if ((strcmp(argv[curArgPos], "-shape") == 0) && numArgsRemaining >= 2) {
      ...
      if (strcmp(argv[curArgPos], "circle") == 0) {
        ...
        numArgsRemaining--;
        curArgPos++;
        opserr << "The procedure for square shapes not implemented yet."
               << endln;
      } else if (strcmp(argv[curArgPos], "rectangle") == 0) {
        shapeSet = true;
        shape = 4;
        numArgsRemaining--;
        curArgPos++;
        opserr << "The procedure for rectangle shapes not implemented yet."
               << endln;
      } else if (strcmp(argv[curArgPos], "hexagon") == 0) {
        shapeSet = true;
        shape = 6;
        numArgsRemaining--;
        curArgPos++;
        opserr << "The procedure for hexagon shapes not implemented yet."
               << endln;
      } else {
        numArgsRemaining--;
        curArgPos++;
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-ele") == 0) &&
               numArgsRemaining >= 2) {
      readingSolidEleTag = true;
      solidEleSetDefined = true;
      numArgsRemaining--;
      ...
      numArgsRemaining--;
      curArgPos++;
    } else if ((strcmp(argv[curArgPos], "-eleRange") == 0) &&
               numArgsRemaining >= 3) {
      readingSolidEleTag = false;
      solidEleSetDefined = true;
      numArgsRemaining--;
  ...
  if (debugFlag)
    for (int ii = 0; ii < numPts; ii++)
      opserr << "point " << ii + 1 << " : " << cXr(ii) << " " << cYr(ii) << " "
             << cZr(ii) << endln;
  ...
  for (int jj = 0; jj < numPts; jj++) {
    for (int ii = 0; ii < solidEleTags.size(); ii++) {
      ...
      for (int kk = 0; kk < 8; kk++) {
        if (debugFlag)
          opserr << "    Node " << kk + 1
                 << " coordinates = " << solidNodesPtr[kk]->getCrds();
      ...
      if (inBounds) {
        ...
        if (lagrangeTag > 1) {
          ...
          if (lagrangeTag == 0) {
            // theElement = new EmbeddedBeamInterface(maxTag, beamTag,
            // solidEleTags[ii], transfTag, loc_rho(jj), loc_theta(jj), xi, eta,
            // zeta, radius, area);
          } else if (lagrangeTag == 1) {
            // theElement = new EmbeddedBeamInterfaceAL(maxTag, beamTag,
            // solidEleTags[ii], transfTag, loc_rho(jj), loc_theta(jj), xi, eta,
  ...
  if (lagrangeTag > 1) {
    if (lagrangeTag == 2) {
      ...
      // theElement = new EmbeddedBeamToeP(maxTag, beamTag,
      // solidEleTagsInContact, transfTag, contactPt_rho, contactPt_theta,
      // contactPt_xi, contactPt_eta, contactPt_zeta, contactPt_radius, radius,
      // area);
    } else if (lagrangeTag == 4) {
      // theElement = new EmbeddedBeamToeP(maxTag, beamTag,
      // solidEleTagsInContact, transfTag, contactPt_rho, contactPt_theta,
...
int
invIsoMapping(Vector nodesX, Vector nodesY, Vector nodesZ, double Px, double Py,
              double Pz, double &xi, double &eta, double &zeta, bool &inBounds)
{
  ...
  for (int ii = 1; ii <= 20; ii++) {
    res(0) = a + a_xi * xi + a_eta * eta + a_zeta * zeta + a_xe * xi * eta +
             a_xz * xi * zeta + a_ez * eta * zeta + a_xez * xi * eta * zeta;
    res(1) = b + b_xi * xi + b_eta * eta + b_zeta * zeta + b_xe * xi * eta +
             b_xz * xi * zeta + b_ez * eta * zeta + b_xez * xi * eta * zeta;
    res(2) = c + c_xi * xi + c_eta * eta + c_zeta * zeta + c_xe * xi * eta +
             c_xz * xi * zeta + c_ez * eta * zeta + c_xez * xi * eta * zeta;

    if (res.Norm() < 1.0e-10) {
      if ((xi < -1) || (xi > 1) || (eta < -1) || (eta > 1) || (zeta < -1) ||
          (zeta > 1))
        inBounds = false;
