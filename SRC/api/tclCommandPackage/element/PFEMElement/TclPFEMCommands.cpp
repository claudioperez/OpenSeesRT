...
int
TclBasicBuilderPFEM2DCommand(ClientData clientData, Tcl_Interp *interp,
...
int
TclBasicBuilderPFEM3DCommand(ClientData clientData, Tcl_Interp *interp,
                             int argc, TCL_Char **argv, Domain *theDomain)
{
          ...
          for (int j = 0; j < (int)fstates.size(); j++) {
            ...
            if (fstate == 1) { // facet hole
              ...
              for (int floc = fl0; floc < fl1; floc++) {
                if (Tcl_GetDouble(interp, argv[floc], &coord[count++]) !=
                    TCL_OK) {
