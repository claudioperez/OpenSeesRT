 ...
 */
#ifdef _TCL85
int (*tclDummyLinkVarPtr)(Tcl_Interp *interp, const char *a, char *b,
                          int c) = Tcl_LinkVar;
#elif _TCL84
int (*tclDummyLinkVarPtr)(Tcl_Interp *interp, const char *a, char *b,
                          int c) = Tcl_LinkVar;
...
TclInterpreter::TclInterpreter(int argc, char **argv) : wrapper(), cmds(this)
{
  ...
  fprintf(stderr, "\t    (c) Copyright 1999,2000 The Regents of the University "
                  "of California");
  fprintf(stderr, "\n\t\t\t\t All Rights Reserved\n");
  fprintf(stderr, "    (Copyright and Disclaimer @ "
  ...
  if (tclStartupScriptFileName == NULL) {
    ...
    tclStartupScriptFileName = Tcl_ExternalToUtfDString(
        NULL, tclStartupScriptFileName, -1, &argString);
  }
...
TclInterpreter::getInt(int *data, int numArgs)
{
  ...
  for (int i = 0; i < numArgs; i++) {
    if (Tcl_GetInt(interp, wrapper.getCurrentArgv()[wrapper.getCurrentArg()],
                   &data[i]) != TCL_OK) {
...
TclInterpreter::getDouble(double *data, int numArgs)
{
  ...
  for (int i = 0; i < numArgs; i++) {
    if (Tcl_GetDouble(interp, wrapper.getCurrentArgv()[wrapper.getCurrentArg()],
                      &data[i]) != TCL_OK) {
      wrapper.incrCurrentArg();
      return -1;
    } else {
