...
// $Source:
// /usr/local/cvs/OpenSees/SRC/reliability/analysis/gFunction/TclGFunEvaluator.cpp,v
// $
...
TclGFunEvaluator::TclGFunEvaluator(Tcl_Interp *passedTclInterp,
    ...
    : GFunEvaluator(), theTclInterp(passedTclInterp),
...
TclGFunEvaluator::setTclRandomVariables(const Vector &x)
{
  ...
  for (int i = 0; i < nrv; i++) {
    ...
    if (Tcl_SetVar2Ex(theTclInterp, "xrv", theIndex, Tcl_NewDoubleObj(xval),
                      TCL_LEAVE_ERR_MSG) == NULL) {
      opserr << "ERROR GFunEvaluator -- error in setTclRandomVariables xrv"
             << endln;
    ...
    if (Tcl_SetVar2Ex(theTclInterp, theIndex, NULL, Tcl_NewDoubleObj(xval),
                      TCL_LEAVE_ERR_MSG) == NULL) {
      opserr << "ERROR GFunEvaluator -- error in setTclRandomVariables x"
             << endln;
      opserr << theTclInterp->result << endln;
      return -1;
    }
...
TclGFunEvaluator::evaluateG(const Vector &x)
{
  ...
  if (this->setTclRandomVariables(x) != 0) {
    opserr
        << "ERROR TclGFunEvaluator::evaluateG -- error in setTclRandomVariables"
        << endln;
    return -1;
  }
  ...
  if (Tcl_ExprDouble(theTclInterp, theExpression, &g) != TCL_OK) {
    opserr << "TclGFunEvaluator::evaluateGnoRecompute -- expression \""
           << theExpression;
...
TclGFunEvaluator::runGFunAnalysis(const Vector &x)
{
  ...
  /*
          // Zero out the response in the structural domain to make ready for
     next analysis char theRevertToStartCommand[10] = "reset"; if
     (Tcl_Eval(theTclInterp, theRevertToStartCommand) == TCL_ERROR) { opserr <<
     "ERROR TclGFunEvaluator -- error in Tcl_Eval for the reset command" <<
     endln; return -1;
          }
  */
  ...
  // revertToStart() is also called on theTransientIntegrator -- MHS needs to
  // check
  ...
  // Put random variables into the structural domain according to the
  // RandomVariablePositioners
  int rvIndex;
  ...
  // Set random variable values in Tcl namespace
  if (this->setTclRandomVariables(x) != 0) {
    opserr << "ERROR TclGFunEvaluator::runGFunAnalysis -- error in "
...
TclGFunEvaluator::tokenizeSpecials(TCL_Char *theExpression, Tcl_Obj *passedList)
{
  ...
  // Note: we no longer need theExpression as all the parameters were saved in
  // LimitStateFunction
  /*
          ...
          if (Tcl_ListObjLength(theTclInterp,paramList,&llength) != TCL_OK) {
                  opserr << "TclGFunEvaluator::tokenizeSpecials ERROR getting
     list length. " << endln; opserr << theTclInterp->result << endln;
          }
          ...
          for (int jk = 0; jk < llength; jk++) {
                  ...
                  if ( strncmp(listStr, "u", 1) == 0 ) {
                          uParse(listStr, &nodeNumber, &direction, dispOrWhat,
     varName, arrName); nodeTclVariable(nodeNumber, direction, dispOrWhat,
     varName, arrName);
                  }
                  else if ( strncmp(listStr, "node", 4) == 0 || strncmp(listStr,
     "rec_node", 8) == 0 ) { nodeParse(listStr, &nodeNumber, &direction,
     dispOrWhat, varName, arrName); nodeTclVariable(nodeNumber, direction,
     dispOrWhat, varName, arrName);
                  }
                  // If an element quantity is detected
                  else if ( strncmp(listStr, "element", 7) == 0 ||
     strncmp(listStr, "rec_element", 11) == 0 ) { elementParse(listStr,
     &eleNumber, varName, eleRest); elementTclVariable(eleNumber, varName,
     eleRest);
                  }
          }
