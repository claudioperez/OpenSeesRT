...
// $Source:
// /usr/local/cvs/OpenSees/SRC/reliability/analysis/gFunction/TclEvaluator.cpp,v
// $
...
TclEvaluator::TclEvaluator(Tcl_Interp *passedTclInterp,
    ...
    : FunctionEvaluator(), theTclInterp(passedTclInterp),
...
TclEvaluator::setVariables()
{
  ...
  for (int i = 0; i < nparam; i++) {
    ...
    if (Tcl_SetVar2Ex(theTclInterp, "par", theIndex, Tcl_NewDoubleObj(xval),
                      TCL_LEAVE_ERR_MSG) == NULL) {
      opserr << "ERROR TclEvaluator -- error in setVariables for parameter tag "
             << paramTag << endln;
...
{
  if (theExpression == 0) {
    opserr << "TclEvaluator::evaluateExpression -- must set the expression "
              "before trying ";
  ...
  if (Tcl_ExprDouble(theTclInterp, theExpression, &current_val) != TCL_OK) {
    opserr << "TclEvaluator::evaluateExpression -- expression \""
           << theExpression;
    opserr << "\" caused error:" << endln << Tcl_GetStringResult(theTclInterp)
           << endln;
    return -1;
  }
...
TclEvaluator::runAnalysis()
{
  ...
  if (fileName == 0) {
    // no source file provided, this is akin to the basic evaluator of days gone
    // by

  } else {
    if (Tcl_Eval(theTclInterp, fileName) == TCL_ERROR) {
      opserr << "ERROR TclEvaluator -- error in Tcl_Eval: "
             << Tcl_GetStringResult(theTclInterp) << endln;
      return -1;
    }

    // make sure the parameter variables in the namespace update to reflect the
...
int
TclEvaluator::setResponseVariable(const char *label, int lsfTag, int rvTag,
                                  double value)
{
  char theIndex[80];
  ...
  if (Tcl_SetVar2Ex(theTclInterp, label, theIndex, Tcl_NewDoubleObj(value),
                    TCL_LEAVE_ERR_MSG) == NULL) {
    opserr << "ERROR TclEvaluator -- error in setResponseVariable for object "
              "with tag "
           << rvTag << endln;
    opserr << "of type " << Tcl_GetStringResult(theTclInterp) << endln;
    return -1;
...
TclEvaluator::getResponseVariable(const char *label, int lsfTag, int rvTag)
{
  ...
  if (value == NULL) {
    opserr << "ERROR TclEvaluator -- error in getResponseVariable for object "
              "with tag "
           << rvTag << endln;
    opserr << "of type " << Tcl_GetStringResult(theTclInterp) << endln;
    return -1;
