                  ...
                  TCL_Char **argv, Domain &theDomain, Recorder **theRecorder)
{
  ...
  if ((strcmp(argv[1], "Element") == 0) ||
    ...
    while (flags == 0 && loc < argc) {
      ...
      if ((strcmp(argv[loc], "-ele") == 0) ||
        ...
        if (eleIDs == 0) {
          opserr << "WARNING recorder Element -eleRange start? end? - out of "
                    "memory\n";
          return TCL_ERROR;
        }
  ...
  ///recorder////////////////////////////
    ...
    int maxNumCriteria =
        2; // current maximum number of either-or criteria for an element
    double dT = 0.0;
    bool echoTime = false;
    int endEleIDs = 2;
    ...
    while (flags == 0 && loc < argc) {
      ...
      //    end of new
        ...
        if (loc < argc &&
            Tcl_GetDouble(interp, argv[loc + 1], &eleM) != TCL_OK) {
          ...
          int i = 0;
          while (loc < argc &&
                 Tcl_GetDouble(interp, argv[loc], &eleM) == TCL_OK) {
            eleMass(i) = eleM;
            loc++;
            i++;
          }
        ...
        if (theRecorderDatabase != 0)
          ...
          opserr << "WARNING recorder Node .. -database &lt;fileName&gt; - NO "
                    "CURRENT DATABASE, results to File instead\n";
          fileName = argv[loc + 1];
        }
  ...
  // create a recorder to write nodal displacement quantities to a file
    ...
    while (flags == 0 && pos < argc) {
      ...
      if (strcmp(argv[pos], "-time") == 0) {
        ...
        // ensure no segmentation fault if user messes up
        if (argc < pos + 3) {
          opserr << "WARNING recorder Node .. -nodeRange start? end?  .. - no "
                    "ele tags specified\n";
          return TCL_ERROR;
        }
  ...
  // a recorder for the graphical display of the domain
    ...
    if (argc < 7) {
      opserr << "WARNING recorder display title xLoc yLoc pixelsX pixelsY "
    ...
    if (argc < 9) {
      opserr << "WARNING recorder plot fileName? windowTitle? xLoc yLoc "
                "xPixels yPixels -columns colX1 colY1 -columns colX2 ...";
      return TCL_ERROR;
    }
