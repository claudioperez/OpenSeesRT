...
#include <SRC/element/elastomericBearing/ElastomericBearingBoucWen3d.h>
void *OPS_ElastomericBearingBoucWen3d()
{
  ...
  if (OPS_GetNumRemainingInputArgs() < 19) {
    ...
    opserr << "Want: elastomericBearingBoucWen eleTag iNode jNode kInit qd "
              "alpha1 alpha2 mu eta beta gamma -P matTag -T matTag -My matTag "
              "-Mz matTag <-orient <x1 x2 x3> y1 y2 y3> <-shearDist sDratio> "
