...
#include <SRC/element/mixedBeamColumn/MixedBeamColumn2d.h>
void *OPS_MixedBeamColumn2d()
{
  ...
  if (OPS_GetNDF() != NND) {
    opserr
        << "ERROR: MixedBeamColumn2d: invalid number of degrees of freedom\n";
    return 0;
  }
  ...
  while (OPS_GetNumRemainingInputArgs() > 0) {
    ...
    if (strcmp(sData, "-mass") == 0) {
      ...
      if (OPS_GetInt(&numData, &doRayleigh) != 0) {
        opserr << "WARNING: Invalid doRayleigh in element MixedBeamColumn2d "
               << eleTag;
        return 0;
      }
  ...
  Element *theElement =
      new MixedBeamColumn2d(eleTag, nodeI, nodeJ, secTags.Size(), sections, *bi,
