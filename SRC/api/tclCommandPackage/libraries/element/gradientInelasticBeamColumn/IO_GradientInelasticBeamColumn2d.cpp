...
#include <SRC/element/gradientInelasticBeamColumn/GradientInelasticBeamColumn2d.h>
void *OPS_GradientInelasticBeamColumn2d()
{
  // Necessary Arguments
  if (OPS_GetNumRemainingInputArgs() < 8) {
    opserr
        << "WARNING! gradientInelasticBeamColumn2d - insufficient arguments\n"
  ...
  int ndf = OPS_GetNDF();
  if (ndm != 2 || ndf != 3) {
    opserr << "WARNING! gradientInelasticBeamColumn2d - ndm must be 2 and ndf "
              "must be 3\n";
    return 0;
  }
    ...
    if (strcmp(word, "-constH") == 0)
      ...
      if (OPS_GetNumRemainingInputArgs() > 2) {
        if (OPS_GetIntInput(&numData, &maxIter) < 0) {
          opserr
              << "WARNING! gradientInelasticBeamColumn2d - invalid maxIter\n";
          return 0;
        }
        ...
        opserr << "WARNING! gradientInelasticBeamColumn2d - need maxIter? "
      ...
      if (OPS_GetNumRemainingInputArgs() > 1) {
        if (OPS_GetDoubleInput(&numData, &maxEpsInc) < 0) {
          opserr
              << "WARNING! gradientInelasticBeamColumn2d - invalid maxEpsInc\n";
          return 0;
        }
        if (OPS_GetDoubleInput(&numData, &maxPhiInc) < 0) {
          opserr
              << "WARNING! gradientInelasticBeamColumn2d - invalid maxPhiInc\n";
          return 0;
        }
      } else
        opserr << "WARNING! gradientInelasticBeamColumn2d - no max. correction "
                  "increments set\n"
  ...
  if (theTransf == 0) {
    opserr << "WARNING! gradientInelasticBeamColumn2d - CrdTransf with tag "
           << transfTag << " not found\n";
    return 0;
  }
  ...
  if (theRule == 0) {
    opserr << "WARNING! gradientInelasticBeamColumn2d - BeamIntegrationRule "
              "with tag "
           << integrTag << " not found\n";
    return 0;
  }
  ...
  if (beamIntegr == 0) {
    opserr << "WARNING! gradientInelasticBeamColumn2d - failed to create beam "
              "integration\n";
    return 0;
  }
  ...
  for (int i = 2; i < numIntegrPoints; i++) {
    if (secTags(i) != secTags(i - 1)) {
      opserr << "WARNING! gradientInelasticBeamColumn2d - internal integration "
  ...
  if (!endSection1) {
    opserr << "WARNING! gradientInelasticBeamColumn2d - section with tag "
           << secTags(0) << " not found\n";
    return 0;
  }
  ...
  Element *theEle = new GradientInelasticBeamColumn2d(
