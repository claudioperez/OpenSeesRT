...
proc BeamWithConcentratedHingesWSection2d {eleTag iNode jNode sectType E Fy transfTag args} { 
 ...
 #              <-release1> <-release2>  
 #              <-release1> and/or <-release2> - activate this option if beam ends are hinged (no bending moment)   
 #              <-nFactor $nFactor>  - Elastic stiffness amplification factor (to make the springs rigid - plastic), default value: 1000 
 #              <-H  $H> Member Length without considering the panel zones (in mm or in), default member length
 #              <-Lb $Lb> Unbraced length from point of plastic hinge location to point of zero moment (in mm or in), default member length
 #              <-compType $Com_Type> - Type of component (Use: "other-than-RBS" for this procedure), default "other-than-RBS"
 #              <-compAction $Comp_Action>  Composite Action flag (Use: 1 (yes), 0 (No) ), default No
 #########################################################################################################        
    ...
    set Comp_Action 0
    if {[lsearch $args "-compAction"] != -1} {
        set loc [lsearch $args "-compAction"]
        set Comp_Action [lindex $args [expr $loc+1]]
    }

    set Com_Type "other-than-RBS"
    if {[lsearch $args "-comType"] != -1} {
        set loc [lsearch $args "-comType"]
        set Com_Type [lindex $args [expr $loc+1]]
    }
    ...
    # add nodes at element ends for rotational springs
    set hingeEnd1 1
    node $eleTag$hingeEnd1 [nodeCoord $iNode 1] [nodeCoord $iNode 2]
    equalDOF $iNode $eleTag$hingeEnd1 1 2
    ...
    if {[lsearch $args "-metric"] != -1} {
        SteelWSectionMR02 $eleTag $E $Fy $H $L $Lb $sectType $Com_Type $Comp_Action -metric -nFactor $nFactor -matType $matType
    } else {
        SteelWSectionMR02 $eleTag $E $Fy $H $L $Lb $sectType $Com_Type $Comp_Action -nFactor $nFactor -matType $matType
    }
    ...
    if {[lsearch $args "-release2"] == -1} {
        element zeroLength $eleTag$hingeEnd2 $jNode  $eleTag$hingeEnd2 -mat $eleTag -dir 6 -doRayleigh $doRayleigh
    }
...
proc BeamWithConcentratedHingesSquareHSS2d {eleTag iNode jNode sectType E Fy N transfTag args} { 

 #########################################################################################################
 #                                                                                                    
 # Creates a Concentrated Plastic Hinge (CPH) element with two zero-length springs at both ends and a
 #                                    linear elastic element in between for square HSS elements                                    
 #                                                                                                    
 # Zero-length springs moment-rotation behavior is defined using the Bilin02 modelusing procedure     
 #   SteelSquareHSSMR02 (see below)                                                                  
 #                                                                                                    
 #
 # Procedure written by: Xin Q., SEP-2017                               
 #
 #  eleTag      - Element ID
 #  iNode       - First node
 #  jNode       - Second node
 ...
 #  E           - Young's modulus (in MPa or ksi)   
 #  Fy          - Yield stress (in MPa or ksi)     
 #  N           - applied axial force due to gravity loads 
 #  transfTag   - geometric transformation     
 #  args        - <YY> 
 #               <-metric>  - activate this option if  in and ksi are used          
 #              <-release1> <-release2>  
...
#       puts "uniaxialMaterial $bilinType $matTag $K_s $as_s $as_s $My_s -$My_s $Lmda $Lmda_c $Lmda $Lmda_k $c_S $c_C $c_A $c_K $theta_p_s $theta_p_s $theta_pc_s $theta_pc_s $Res_s $Res_s $theta_ult_s $theta_ult_s 1.  1. $nFactor"
    ...
    puts "not defined"
      ...
      # TODO
      uniaxialMaterial "[lindex $matType 0]" $matTag {*}[lrange $matType 1 end]
    }
}
